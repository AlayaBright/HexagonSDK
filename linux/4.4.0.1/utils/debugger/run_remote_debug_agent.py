...
def setup_and_start_remote_debug_agent():
    ...
    try:
        output1 = subprocess.check_output(rdbg_system_ls, stderr = subprocess.STDOUT, shell = True).decode('utf-8')
    ...
    try:
        output2 = subprocess.check_output(rdbg_vendor_ls, stderr = subprocess.STDOUT, shell = True).decode('utf-8')
    except Exception as e:
        found_vendor = 0
        output2 = str(e)
    else:
    ...
    try:
        output1 = subprocess.check_output(rdbg_system_insmod, stderr = subprocess.STDOUT, shell = True).decode('utf-8')
    except Exception as e:
        output1 = str(e)

    try:
        output2 = subprocess.check_output(rdbg_vendor_insmod, stderr = subprocess.STDOUT, shell = True).decode('utf-8')
    except Exception as e:
        output2 = str(e)

    # check to see if rdbg.ko module is live -> if not then rdbg.ko not instantiated properly
