...
parser=OptionParser(usage="usage: %prog [-e --hexagonEclipse] [-Z --hexagonZip] [-V --vm]",)    # version="%prog 1.0")
...
def parse_input():
    ...
    global installer
    global cmake4eclipse_installer
    installer = options.zip_install
    if not uninstall:
        cmake4eclipse_installer = installer
        if cmake4eclipse_installer:
            cmake4eclipse_installer = cmake4eclipse_installer.replace("IDE.zip", "cmake4eclipse-2.1.3.zip")
            #print(cmake4eclipse_installer)
    #os.path.abspath('../../tools/hexagon_ide/ide_plugins/photon/cmake4eclipse-2.1.3.zip')
...
def install_plugins(version):
    ...
    if not java_loc:
        java_loc = "../../tools/hexagon_ide/jre/bin"
    if not uninstall:
        ...
        final_installer=""
        final_cmake4eclipse_installer=""
        global installer
        global cmake4eclipse_installer
        if not installer:
            if(version=="photon"):
                installer = os.path.abspath('../../tools/hexagon_ide/ide_plugins/photon/IDE.zip')
                cmake4eclipse_installer = installer
                cmake4eclipse_installer = cmake4eclipse_installer.replace("IDE.zip", "cmake4eclipse-2.1.3.zip")
                #os.path.abspath('../../tools/hexagon_ide/ide_plugins/photon/cmake4eclipse-2.1.3.zip')
        if(installer.startswith('http')):
            if(sys.version_info[0] < 3):
                ...
                parts = urllib.parse.urlsplit(installer)
            if not parts.scheme or not parts.netloc:
                sys.exit('Not an valid url')
            final_installer = installer
            final_cmake4eclipse_installer = cmake4eclipse_installer
        else:
            ...
            if 'windows' in osType.lower():
                ...
                final_cmake4eclipse_installer = "jar:file:/" + cmake4eclipse_installer + "!/"
            else:
                if installer.startswith('/'):
                    ...
                    final_installer = "jar:file:/" + installer + "!/"
                    final_cmake4eclipse_installer = "jar:file:/" + cmake4eclipse_installer + "!/"
        final_installer = final_installer.replace('\\', '/')
        final_cmake4eclipse_installer = final_cmake4eclipse_installer.replace('\\', '/')
        print("Installing Hexagon plugins...")
        print("Please wait...")
        #uninstall '
        uninstall_plugins(0)
        #install Cmake4Eclipse P2 install
        print('Cmake4Eclipse IDE: Installing from ' + final_cmake4eclipse_installer)
        proc=subprocess.Popen([eclipse_exe, '-vm', java_exe, '-application', 'org.eclipse.equinox.p2.director', '-repository', final_cmake4eclipse_installer, '-installIU', 'de.marw.cdt.cmake.feature.group' ,'-noSplash'],stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = proc.communicate()
        display_stream(out,err);
        print('Cmake4Eclipse IDE:dependencies Installation Done...')
        #now install
        print('Hexagon plugin Installation from ' + final_installer)
...
def install_android():
    ...
    if eclipse_location:
        ...
        download_android=input('SDK will be downloaded at: '+ os.path.abspath(fileName) + "\nDownload SDK? [Press Y to download, N to ignore it] ").lower()
        ...
        if not is_android_downloaded:
            if(sys.version_info[0] < 3):
                ...
                u = urllib.request.urlopen(androidURL)
        ...
        ndkURL=''
        download_ndk=input('Download NDK? Choose a option:\n1.32bit NDK\t2.64bit NDK\t3.Don\'t download NDK\n')
        if download_ndk=='1':
        ...
        if ndkURL:
            ...
            if(sys.version_info[0] < 3):
                ...
                u = urllib.request.urlopen(ndkURL)
        ...
        if os.path.exists(eclipse_location+"/eclipse.exe"):
            ...
            install_cmd = eclipse_location + "/eclipse.exe -application org.eclipse.equinox.p2.director -repository http://download.eclipse.org/releases/photon -installIU org.eclipse.jdt"
            subprocess.call(install_cmd)
            print("org.eclipse.jdt installed")

            install_cmd = eclipse_location + "/eclipse.exe -application org.eclipse.equinox.p2.director -repository http://download.eclipse.org/releases/photon -installIU org.eclipse.jdt.junit"
            subprocess.call(install_cmd)
            print("org.eclipse.jdt.junit installed")

            install_cmd = eclipse_location + "/eclipse.exe -application org.eclipse.equinox.p2.director -repository http://download.eclipse.org/releases/photon -installIU org.eclipse.wst.sse.core"
            subprocess.call(install_cmd)
            print("org.eclipse.wst.sse.core installed")

            install_cmd = eclipse_location + "/eclipse.exe -application org.eclipse.equinox.p2.director -repository http://download.eclipse.org/releases/photon -installIU org.eclipse.wst.sse.ui"
            subprocess.call(install_cmd)
            print("org.eclipse.wst.sse.ui installed")

            install_cmd = eclipse_location + "/eclipse.exe -application org.eclipse.equinox.p2.director -repository http://download.eclipse.org/releases/photon -installIU org.eclipse.wst.xml.core"
            subprocess.call(install_cmd)
            print("org.eclipse.wst.xml.core installed")

            install_cmd = eclipse_location + "/eclipse.exe -application org.eclipse.equinox.p2.director -repository http://download.eclipse.org/releases/photon -installIU org.eclipse.wst.xml.ui"
            subprocess.call(install_cmd)
