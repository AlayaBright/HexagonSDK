    ...
    for file_name in os.listdir(spath):
        ...
        if(os.path.isfile(s_file)):
            try:
                ...
                binaryFiles.append(s_file)
                pass
        else:
            if "android_" not in s_file and "hexagon_" not in s_file and "UbuntuARM_" not in s_file and ".git" not in s_file:
...
print('src path is : {}'.format(sys.argv[1]));
