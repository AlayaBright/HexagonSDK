...
#******************************************************************************
parser = argparse.ArgumentParser(prog='walkthrough.py', description=__doc__, formatter_class=argparse.RawTextHelpFormatter)
...
def run_example():
    ...
    run_main_on_hexagon_exe = '{}/libs/run_main_on_hexagon/{}/ship/run_main_on_hexagon'.format(HEXAGON_SDK_ROOT,hlos_dir)
    librun_main_on_hexagon_skel = '{}/libs/run_main_on_hexagon/{}/ship/librun_main_on_hexagon_skel.so'.format(HEXAGON_SDK_ROOT,dsp_dir)
    test_main = '{}/libs/run_main_on_hexagon/{}/ship/test_main.so'.format(HEXAGON_SDK_ROOT,dsp_dir)

    if sys.platform == "win32":
        clean_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/libs/run_main_on_hexagon '+variant+'_clean HLOS_ARCH='+arch+' VERBOSE=1 || exit /b'
        build_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/libs/run_main_on_hexagon '+variant+' HLOS_ARCH='+arch+' VERBOSE=1 || exit /b'
        clean_hexagon_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/libs/run_main_on_hexagon hexagon_clean DSP_ARCH='+hex_variant+' VERBOSE=1 || exit /b'
        build_hexagon_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/libs/run_main_on_hexagon hexagon DSP_ARCH='+hex_variant+' VERBOSE=1 || exit /b'
    else:
        clean_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/libs/run_main_on_hexagon '+variant+'_clean HLOS_ARCH='+arch+' VERBOSE=1 || exit 1'
        build_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/libs/run_main_on_hexagon '+variant+' HLOS_ARCH='+arch+' VERBOSE=1 || exit 1'
        clean_hexagon_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/libs/run_main_on_hexagon hexagon_clean DSP_ARCH='+hex_variant+' VERBOSE=1 || exit 1'
        build_hexagon_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/libs/run_main_on_hexagon hexagon DSP_ARCH='+hex_variant+' VERBOSE=1 || exit 1'

    if not options.no_rebuild:
        print("---- Build run_main_hexagon example for both Android and Hexagon ----")
        ...
        print("---- Skip rebuilding run_main_hexagon example for both Android and Hexagon ----")
    ...
    dev_conf.print_and_run_cmd('adb -s '+device_number+' wait-for-device shell chmod 777 '+APPS_DST+'/run_main_on_hexagon')
    ...
    dev_conf.print_and_run_cmd('adb -s '+device_number+' wait-for-device push '+test_main +' '+DSP_DST)
    ...
    dev_conf.print_and_run_cmd('adb -s '+device_number+' shell export LD_LIBRARY_PATH='+LIB_DST+' '+DSP_LIB_SEARCH_PATH+'='+DSP_LIB_PATH+' '+APPS_DST+'/run_main_on_hexagon '+ options.domain +' test_main.so 1 foo2 2 bar')
    print("Done")
