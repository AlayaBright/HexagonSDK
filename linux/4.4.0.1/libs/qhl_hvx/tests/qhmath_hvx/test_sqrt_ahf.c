...
@brief
    Accuracy test for qhmath_hvx_sqrt_ahf function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (__fp16)sqrtf(input_arr[i]);
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A1(qhmath_hvx_sqrt_ahf)
    ...
    for (uint32_t i = 1; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_sqrt_ahf(input_arr, output, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            if (error > tolerance)
            {
                printf("Error:\tx = {%.7f} | qhmath_hvx_sqrt_ahf(x) = %.7f | OK = %.7f "
