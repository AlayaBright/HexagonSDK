...
@brief
    Accuracy test for qhmath_hvx_tan_ahf function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (__fp16)tanf((float)input[i]);
    }
}
...
int main(void)
{
    ...
    printf("Test for qhmath_hvx_tan_ahf function:\n");
    ...
    for (uint32_t i = 1; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_tan_ahf(input, output, i);
