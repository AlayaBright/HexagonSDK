/**=============================================================================
@file
   test_aqf16_convert_ah.c

@brief
    Accuracy test for qhmath_hvx_aqf16_convert_ah function
...
int main() {
    ...
    // Use the int16 data to convert to qf16 using intrinsic code and reference code

    qhmath_hvx_aqf16_convert_ah_ref(arg, qf_ref, SIZE);
    qhmath_hvx_aqf16_convert_ah(arg, qf_test, SIZE);

    //compare bit accuracy  of int16 conversion to qfloat16

    if(qhmath_hvx_cmp_qf16(qf_ref, qf_test, SIZE) < 10.0) {
       printf("Conversion: int16 -> qf16 mismatches with reference implementation, FAIL!\n");
    } else {
       printf("Conversion: int16 -> qf16 matches with reference implementation, PASS! \n");
    }
/*======================================================================*/
    free(arg);
    free(qf_ref);
