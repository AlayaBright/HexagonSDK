...
@brief
    Accuracy test for qhmath_hvx_atan_ahf function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (__fp16) atanf((float)input[i]);
    }
}
...
int main(void)
{
    ...
    printf("Test for qhmath_hvx_atan_ahf function:\n");
    ...
    for (uint32_t i = ARR_LEN; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_atan_ahf(input, output, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            {
                printf("Error:\tx = { %.7f } | "
                       "qhmath_hvx_atan_vhf(x) = %.7f | OK = %.7f "
                       "| error_percent = %2.5f "
