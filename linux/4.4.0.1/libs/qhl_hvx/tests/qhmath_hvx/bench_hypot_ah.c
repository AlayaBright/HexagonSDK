...
@brief
    Benchmark for measuring performance of qhmath_hvx_hypot_ah function.

    Benchmark measures number of cycles needed for executing
    qhmath_hvx_hypot_ah function certain number of times
...
static int16_t input_arr_1[VEC_MAX_ARR_LEN];
...
static void clear_cache(uint32_t length)
{
    ...
    hexagon_buffer_cleaninv((uint8_t *)input_arr_1, length*sizeof(int16_t));
#endif
}
...
BENCH_TFUN(qhmath_hvx_hypot_ah, input_arr, input_arr_1, output)
...
int main(void)
{
    ...
    INIT_INPUT_INT16(input_arr_1, VEC_MAX_ARR_LEN, ranges0, RANGE_CNT0, RAND_MAX)

    BENCH_TCALL(qhmath_hvx_hypot_ah, NR_OF_ITERATIONS, lengths_of_arrays, VEC_LEN)
