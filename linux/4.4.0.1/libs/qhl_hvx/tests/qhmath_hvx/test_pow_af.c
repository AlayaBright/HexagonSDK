...
@brief
    Accuracy test for qhmath_hvx_pow_af function
...
static float x_arr[ARR_LEN];
static float y_arr[ARR_LEN];
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = qhmath_pow_f(x_arr[i], y_arr[i]);
    }
}
...
int main(void)
{
    ...
    printf("Test for qhmath_hvx_pow_af function:\n");

    INIT_INPUT_FLOAT(x_arr, ARR_LEN, ranges0, RANGE_CNT0, RAND_MAX)
    INIT_INPUT_FLOAT(y_arr, ARR_LEN, ranges1, RANGE_CNT1, RAND_MAX)
    ...
    for (uint32_t i = 1; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_pow_af(x_arr, y_arr, output, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            if (error > tolerance)
            {
                printf("Error:\tin = %f %f | qhmath_hvx_pow_af = %.7f "
