...
@brief
    Benchmark for measuring performance of qhmath_hvx_pow_ahf function.

    Benchmark measures number of cycles needed for executing
    qhmath_hvx_pow_ahf function certain number of times
...
static __fp16 x_arr[VEC_MAX_ARR_LEN];
static __fp16 y_arr[VEC_MAX_ARR_LEN];
static __fp16 output[VEC_MAX_ARR_LEN];

static void clear_cache(uint32_t length)
{
#if __hexagon__
    hexagon_buffer_cleaninv((uint8_t *)x_arr, length*sizeof(__fp16));
    hexagon_buffer_cleaninv((uint8_t *)y_arr, length*sizeof(__fp16));
#endif
}
...
BENCH_TFUN(qhmath_hvx_pow_ahf, x_arr, y_arr, output)

int main(void)
{
    BENCH_VECTOR_INIT_INPUT(lengths_of_arrays, VEC_LEN, vector_ranges, RANGE_VECTOR, RAND_MAX)

    INIT_INPUT_FLOAT16(x_arr, VEC_MAX_ARR_LEN, ranges0, RANGE_CNT0, RAND_MAX)
    INIT_INPUT_FLOAT16(y_arr, VEC_MAX_ARR_LEN, ranges1, RANGE_CNT1, RAND_MAX)

    BENCH_TCALL(qhmath_hvx_pow_ahf, NR_OF_ITERATIONS, lengths_of_arrays, VEC_LEN)
