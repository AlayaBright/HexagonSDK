...
@brief
    Accuracy test for qhmath_hvx_ceil_ahf function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (__fp16)ceilf((float)input_arr[i]);
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A1(qhmath_hvx_ceil_ahf)

    TEST_INPUT_QFLOAT_A1(qhmath_hvx_ceil_ahf)
