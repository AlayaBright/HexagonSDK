/**=============================================================================
@file
    test_auh_convert_aqf32.c

@brief
    Accuracy test for qhmath_hvx_auh_convert_aqf32 function
...
int main() {
    ...
    qhmath_hvx_auh_convert_aqf32_ref(qf_ref, int_ref, SIZE);
    qhmath_hvx_auh_convert_aqf32(qf_ref, int_test, SIZE);

    //compare bit accuracy  of qfloat conversion to uint16

    if(qhmath_hvx_cmp_uh(int_ref, int_test, SIZE, 1)) {
       printf("Conversion: qf32 -> uint16 mismatches with reference implementation, FAIL!\n");
    } else {
       printf("Conversion: qf32 -> uint16  matches with reference implementation, PASS! \n");
    }
/*======================================================================*/
    free(arg);
    free(int_test);
