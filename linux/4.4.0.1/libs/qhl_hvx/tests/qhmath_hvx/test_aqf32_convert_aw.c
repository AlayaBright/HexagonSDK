...
int main() {
    ...
    qhmath_hvx_aqf32_convert_aw(arg, qf_test, SIZE);
    //compare bit accuracy  of int32_t conversion to qfloat32

    if(qhmath_hvx_cmp_qf32(qf_ref, qf_test, SIZE) < 23.0) {
       printf("Conversion: int32 -> qf32 mismatches with reference implementation, FAIL!\n");
    } else {
       printf("Conversion: int32 -> qf32 matches with reference implementation, PASS! \n");
    }
/*======================================================================*/
    free(arg);
    free(qf_ref);
