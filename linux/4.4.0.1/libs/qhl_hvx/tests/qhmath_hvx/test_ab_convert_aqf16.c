/**=============================================================================
@file
   test_ab_convert_aqf16.c

@brief
    Accuracy test for qhmath_hvx_ab_convert_aqf16 function
...
int main() {
    ...
    qhmath_hvx_aqf16_convert_ab_ref(arg, qf_ref, SIZE);

    // Use the Qfloat data to convert to int8 using intrinsic code and reference code

    qhmath_hvx_ab_convert_aqf16_ref (qf_ref, int_ref, SIZE);
    qhmath_hvx_ab_convert_aqf16 (qf_ref, int_test, SIZE);

    //compare bit accuracy  of qfloat conversion to int8

    if(qhmath_hvx_cmp_b(int_ref, int_test, SIZE, 1)) {
       printf("Conversion: qf16 -> int8 mismatches with reference implementation, FAIL!\n");
    } else {
       printf("Conversion: qf16 -> int8  matches with reference implementation, PASS! \n");
    }
/*======================================================================*/
    free(arg);
    free(int_test);
