...
@brief
    Accuracy test for qhmath_hvx_sqrt_af function
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A1(qhmath_hvx_sqrt_af)
    ...
    for (uint32_t i = 1; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_sqrt_af(input_arr, output, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            if (error > tolerance)
            {
                printf("Error:\tx = {%.7f} | qhmath_hvx_sqrt_af(x) = %.7f | OK = %.7f "
