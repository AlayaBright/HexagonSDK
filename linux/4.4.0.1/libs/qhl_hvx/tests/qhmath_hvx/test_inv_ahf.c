...
@brief
    Accuracy test for qhmath_hvx_inv_ahf function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (__fp16)(1.0f/(float)input_arr[i]);
    }
}
...
int main(void)
{
    ...
    printf("Test for qhmath_hvx_inv_ahf function:\n");
    ...
    for (uint32_t i = 1; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_inv_ahf(input_arr, output, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            if (error > tolerance)
            {
                printf("Error:\tin = %.7f | qhmath_hvx_inv_ahf = %.7f "
