...
@brief
    Accuracy test for qhmath_hvx_div_ahf function
...
static __fp16 n_arr[ARR_LEN];
static __fp16 d_arr[ARR_LEN];
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (__fp16)((float)n_arr[i]/(float)d_arr[i]);
    }
}
...
int main(void)
{
    ...
    printf("Test for qhmath_hvx_div_ahf function:\n");

    INIT_INPUT_FLOAT16(n_arr, ARR_LEN, ranges0, RANGE_CNT0, RAND_MAX)
    INIT_INPUT_FLOAT16(d_arr, ARR_LEN, ranges1, RANGE_CNT1, RAND_MAX)
    ...
    for (uint32_t i = 1; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_div_ahf(n_arr, d_arr, output, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            if (error > tolerance)
            {
                printf("Error:\tin = %.7f %.7f | qhmath_hvx_div_ahf = %.7f "
                       "| OK = %.7f | vector length = %lu | iter = %lu\n",
                       n_arr[j], d_arr[j], output[j], ref_output[j], i, j);
                ++fail;
            }
            else
            {
