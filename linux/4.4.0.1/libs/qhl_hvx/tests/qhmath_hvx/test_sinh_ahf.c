...
@brief
    Accuracy test for qhmath_hvx_sinh_ahf function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (__fp16)sinh((float)input[i]);
    }
}
...
int main(void)
{
    ...
    float tolerance = 6.5;
#ifdef DUMP_DATA
    char *output_filename = "outputs_sinh_ahf.txt";
    FILE *fp_output;
    fp_output = fopen(output_filename, "w");
#endif

    printf("Test for qhmath_hvx_sinh_ahf function:\n");
    ...
    for (uint32_t i = ARR_LEN; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_sinh_ahf(input, output, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            float error_percent;
            if (ref_output[j] != 0) {
                error_percent = fabsf((ref_output[j]-output[j])/(ref_output[j]))*100.0f;
            } else {
                // Treating Absolute error as the % error when reference output is 0
                error_percent = fabsf(ref_output[j]-output[j]) * 100.0f;
            }

            if (error_percent > tolerance)
            {
                printf("Error:\tx = { %.7f } | "
                       "qhmath_hvx_sinh_ahf(x) = %.7e | OK = %.7e "
                       "| error_percent = %2.5f "
                       "| vector length = %u | i = %u\n",
                       input[j], output[j],
                       ref_output[j],
