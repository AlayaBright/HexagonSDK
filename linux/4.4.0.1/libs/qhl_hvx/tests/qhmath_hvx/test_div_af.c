...
@brief
    Accuracy test for qhmath_hvx_div_af function
...
static float n_arr[ARR_LEN];
static float d_arr[ARR_LEN];
...
int main(void)
{
    ...
    printf("Test for qhmath_hvx_div_af function:\n");

    INIT_INPUT_FLOAT(n_arr, ARR_LEN, ranges0, RANGE_CNT0, RAND_MAX)
    INIT_INPUT_FLOAT(d_arr, ARR_LEN, ranges1, RANGE_CNT1, RAND_MAX)
    ...
    for (uint32_t i = 1; i <= ARR_LEN; ++i)
    {
        ...
        qhmath_hvx_div_af(n_arr, d_arr, output, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            if (error > tolerance)
            {
                printf("Error:\tin = %.7f %.7f | qhmath_hvx_div_af = %.7f "
