...
@brief
    Accuracy test for qhmath_hvx_copysign_ahf function
...
static __fp16 y_arr[2] = { -1.0f, 1.0f };
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        __fp16 abs_val = input_arr[i] < 0 ? -input_arr[i] : input_arr[i];
...
int main(void)
{
    ...
    printf("Test for qhmath_hvx_copysign_ahf function:\n");
    ...
    for (uint32_t k = 0; k < YARR_CNT; ++k)
    {
        ...
        for (uint32_t i = 1; i <= ARR_LEN; ++i)
        {
            ...
            qhmath_hvx_copysign_ahf(input_arr, &y, output, i);
            ...
            for (uint32_t j = 0; j < i; ++j)
            {
                if (ref_output[k][j] != output[j])
                {
                    printf("Error:\tin = %.7f | y = %.7f "
                           "| qhmath_hvx_copysign_ahf = %.7f | OK = %.7f "
