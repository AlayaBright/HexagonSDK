...
@brief
    Accuracy test for qhmath_hvx_floor_ahf function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (__fp16)floorf((float)input_arr[i]);
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A1(qhmath_hvx_floor_ahf)

    TEST_INPUT_QFLOAT_A1(qhmath_hvx_floor_ahf)
