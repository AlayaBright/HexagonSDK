...
@brief
    Accuracy test for qhmath_hvx_ceil_af function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = ceilf(input_arr[i]);
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A1(qhmath_hvx_ceil_af)

    TEST_INPUT_QFLOAT_A1(qhmath_hvx_ceil_af)
