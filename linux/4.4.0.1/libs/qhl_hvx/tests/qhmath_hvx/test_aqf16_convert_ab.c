/**=============================================================================
@file
   test_aqf16_convert_ab.c

@brief
    Accuracy test for qhmath_hvx_aqf16_convert_ab function
...
int main() {
    ...
    qhmath_hvx_aqf16_convert_ab(arg, qf_test, SIZE);

    //compare bit accuracy  of int8 conversion to qfloat16

    if(qhmath_hvx_cmp_qf16(qf_ref, qf_test, SIZE) < 10.0) {
       printf("Conversion: int8 -> qf16 mismatches with reference implementation, FAIL!\n");
    } else {
       printf("Conversion: int8 -> qf16 matches with reference implementation, PASS! \n");
    }
/*======================================================================*/
    free(arg);
    free(qf_ref);
