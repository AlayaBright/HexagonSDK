...
/**=============================================================================
@file
    test_aub_convert_aqf32.c

@brief
    Accuracy test for qhmath_hvx_aub_convert_aqf32 function
...
int main() {
    ...
    qhmath_hvx_aub_convert_aqf32_ref(qf_ref, int_ref, SIZE);
    qhmath_hvx_aub_convert_aqf32(qf_ref, int_test, SIZE);

    //compare bit accuracy  of qfloat conversion to uint8

    if(qhmath_hvx_cmp_ub(int_ref, int_test, SIZE, 1)) {
       printf("Conversion: qf32 -> uint8 mismatches with reference implementation, FAIL!\n");
    } else {
       printf("Conversion: qf32 -> uint8  matches with reference implementation, PASS! \n");
    }
/*======================================================================*/
    free(arg);
    free(int_test);
