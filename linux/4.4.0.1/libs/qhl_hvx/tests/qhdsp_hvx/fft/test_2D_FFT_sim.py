...
for fft_window_size in FFT_window_sizes:
    input_file = "../../../../qhdsp_hvx/fft/test_data/2D_FFT_inputs/fft2D_fxp_real_test_data_HVX_in"
    input_file += fft_window_size + "x" + fft_window_size + ".dat"
    ref_file = "../../../../qhdsp_hvx/fft/test_data/2D_FFT_refs/fft2D_fxp_real_test_data_HVX_out"
    ref_file += fft_window_size + "x" + fft_window_size + "_REF.dat"
    dut_file = "../../../../qhdsp_hvx/fft/test_data/2D_FFT_dut/fft2D_fxp_real_test_data_HVX_out"
    dut_file += fft_window_size + "x" + fft_window_size + "_DUT.dat"

    test_cmd_line = hexagon_sim_cmd_line + "test_bench_fxp_2d_fft_real_HVX_qhl -- " + \
                        "-i" + input_file + \
                        " -r" + ref_file + \
                        " -w" + fft_window_size
    if(create_DUT == True):
