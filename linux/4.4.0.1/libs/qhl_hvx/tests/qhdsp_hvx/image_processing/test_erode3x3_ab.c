...
@brief
    Accuracy test for qhdsp_hvx_erode3x3_ab function.
...
static void qhdsp_hvx_erode3x3_ab_ref(
    uint8_t *input,
    int32_t stride_i,
    uint32_t width,
    uint32_t height,
    ...
    for (int32_t i = 1; i < height-1; ++i)
    {
        for (int32_t j = 1; j < width-1; ++j)
        {
            ...
            for (int32_t k = 0; k < 3; ++k)
            {
                ...
                {
                    uint8_t pixel = input[(i-1+k)*stride_i + (j-1+n)];
...
int main(int argc, char* argv[])
{
    ...
    printf("Test for qhdsp_hvx_erode3x3_ab function:\n");
    ...
    /*  Call test fuctions                                  */
    /* -----------------------------------------------------*/
    qhdsp_hvx_erode3x3_ab(input, stride, width, height, output, stride);
    qhdsp_hvx_erode3x3_ab_ref(input, stride, width, height, output_ref, stride);
