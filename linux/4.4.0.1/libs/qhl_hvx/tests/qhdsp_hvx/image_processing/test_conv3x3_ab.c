...
@brief
    Accuracy test for qhdsp_hvx_conv3x3_ab function.
...
static void qhdsp_hvx_conv3x3_ab_ref(
    uint8_t  *input,
    int32_t  stride_i,
    uint32_t width,
    uint32_t height,
    ...
    int32_t  stride_o)
{
    if ((input == NULL) || (mask == NULL) || (output == NULL) || (height < 3)
    ...
    for (int32_t i = 1; i < height-1; ++i)
    {
        for (int32_t j = 1; j < width-1; ++j)
        {
            ...
            for (int32_t k = 0; k < 3; ++k)
            {
                ...
                {
                    int16_t pixel = input[(i-1+k)*stride_i + (j-1+n)];
                    int16_t maskv = mask[k*3+n];
                    sum += pixel * maskv;
                }
            }
...
int main(int argc, char* argv[])
{
    ...
    printf("Test for qhdsp_hvx_conv3x3_ab function:\n");
    ...
    /*  Call test fuctions                                  */
    /* -----------------------------------------------------*/
    qhdsp_hvx_conv3x3_ab(input, stride, width, height, mask, 4, output, stride);
    qhdsp_hvx_conv3x3_ab_ref(input, stride, width, height, mask, 4, output_ref, stride);
