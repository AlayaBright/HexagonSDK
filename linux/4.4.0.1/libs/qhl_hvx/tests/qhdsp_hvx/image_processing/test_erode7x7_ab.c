...
@brief
    Accuracy test for qhdsp_hvx_erode7x7_ab function.
...
static void qhdsp_hvx_erode7x7_ab_ref(
    uint8_t *input,
    int32_t stride_i,
    uint32_t width,
    uint32_t height,
    ...
    int32_t stride_o)
{
    if ((input == NULL) || (output == NULL) || (height < 7)
    ...
    for (int32_t i = 3; i < height-3; ++i)
    {
        for (int32_t j = 3; j < width-3; ++j)
        {
            ...
            for (int32_t k = 0; k < 7; ++k)
            {
                ...
                {
                    uint8_t pixel = input[(i-3+k)*stride_i + (j-3+n)];
...
int main(int argc, char* argv[])
{
    ...
    printf("Test for qhdsp_hvx_erode7x7_ab function:\n");
    ...
    /*  Call test fuctions                                  */
    /* -----------------------------------------------------*/
    qhdsp_hvx_erode7x7_ab(input, stride, width, height, output, stride);
    qhdsp_hvx_erode7x7_ab_ref(input, stride, width, height, output_ref, stride);
