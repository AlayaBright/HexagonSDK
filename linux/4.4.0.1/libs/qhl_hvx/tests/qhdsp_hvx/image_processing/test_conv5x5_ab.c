...
@brief
    Accuracy test for qhdsp_hvx_conv5x5_ab function.
...
static void qhdsp_hvx_conv5x5_ab_ref(
    uint8_t  *input,
    int32_t  stride_i,
    uint32_t width,
    uint32_t height,
    ...
    int32_t  stride_o)
{
    if ((input == NULL) || (mask == NULL) || (output == NULL) || (height < 5)
    ...
    for (int32_t i = 2; i < height-2; ++i)
    {
        for (int32_t j = 2; j < width-2; ++j)
        {
            ...
            for (int32_t k = 0; k < 5; ++k)
            {
                ...
                {
                    int16_t pixel = input[(i-2+k)*stride_i + (j-2+n)];
                    int16_t maskv = mask[k*5+n];
                    sum += pixel * maskv;
                }
            }
...
int main(int argc, char* argv[])
{
    ...
    printf("Test for qhdsp_hvx_conv5x5_ab function:\n");
    ...
    /*  Call test fuctions                                  */
    /* -----------------------------------------------------*/
    qhdsp_hvx_conv5x5_ab(input, stride, width, height, mask, 4, output, stride);
    qhdsp_hvx_conv5x5_ab_ref(input, stride, width, height, mask, 4, output_ref, stride);
