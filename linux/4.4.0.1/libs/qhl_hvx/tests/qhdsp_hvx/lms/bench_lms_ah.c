...
@brief
    Benchmark for measuring performance of qhdsp_hvx_lms_ah function.

    Benchmark measures number of cycles needed for executing
    qhdsp_hvx_lms_ah function certain number of times
...
int main(void)
{
    ...
    printf("Benchmark for qhdsp_hvx_lms_ah function:\n");
    ...
    for (uint32_t i = 256; i <= ARR_LEN; i += i)
    {
        ...
        for (uint32_t j = 64; j <= i; j += j)
        {
            cpc = 0;
            uspc = 0;
            memset(states, 0, sizeof(int16_t)*ARR_LEN);
            memcpy(coeffs, orig_coeffs, sizeof(int16_t)*ARR_LEN);

            for (uint32_t k = 0; k < NR_OF_ITERATIONS; ++k)
            {
                hexagon_buffer_cleaninv((uint8_t *)input, i*sizeof(int16_t));
                measurement_start();
                qhdsp_hvx_lms_ah(input, ydes, rate, j, states,
