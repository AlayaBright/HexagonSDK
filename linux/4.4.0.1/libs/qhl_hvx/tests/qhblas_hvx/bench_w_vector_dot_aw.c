/**=============================================================================
@file
    bench_w_vector_dot_aw.c

@brief
    Benchmark for measuring performance of qhblas_hvx_w_vector_dot_aw function.

    Benchmark measures number of cycles needed for executing
    qhblas_hvx_w_vector_dot_aw function certain number of times
...
static int32_t input_arr2[VEC_MAX_ARR_LEN];
...
static void clear_cache(uint32_t length)
{
    ...
    hexagon_buffer_cleaninv((uint8_t *)input_arr2, length*sizeof(int32_t));
#endif
}
...
BENCH_TFUN(qhblas_hvx_w_vector_dot_aw, input_arr1, input_arr2, &output)
...
int main(void)
{
    ...
    INIT_INPUT_INT32(input_arr2, VEC_MAX_ARR_LEN, ranges0, RANGE_CNT0, RAND_MAX)

    BENCH_TCALL(qhblas_hvx_w_vector_dot_aw, NR_OF_ITERATIONS, lengths_of_arrays, VEC_LEN)
