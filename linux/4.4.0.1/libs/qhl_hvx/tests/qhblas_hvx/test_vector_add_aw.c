/**=============================================================================
@file
    test_vector_add_aw.c

@brief
    Accuracy test for qhblas_hvx_vector_add_aw function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        int64_t sum = input_arr1[i] + input_arr2[i];
        sum = ((sum > 2147483647)
               ? 2147483647
               : ((sum < -2147483647) ? -2147483648 : sum));
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A2(qhblas_hvx_vector_add_aw)

    TEST_INPUT_AH2(qhblas_hvx_vector_add_aw)
