/**=============================================================================
@file
    test_matrix_matrix_mpy_ahf.c

@brief
    Accuracy test for qhblas_hvx_matrix_matrix_mpy_ahf function
...
inline static void generate_ref_output(uint32_t n)
{
    ...
    for (size_t i = 0; i < M; i++)
    {
        for (size_t j = 0; j < n; j++)
        {
            ...
            {
                ref_output[i * n + j] += (float)input_mtx1[i * K + k] * (float)input_mtx2[k * n + j];
            }
        }
    }
}
...
int main(void)
{
    ...
    printf("Test for qhblas_hvx_matrix_matrix_mpy_ahf function:\n");
    ...
    for (uint32_t i = 1; i <= N; ++i)
    {
        ...
        qhblas_hvx_matrix_matrix_mpy_ahf(input_mtx1, input_mtx2, output, M, K, i);
        ...
        for (uint32_t j = 0; j < M * i; ++j)
        {
            ...
            if (error > TOLERANCE)
            {
                printf("Error: qhblas_hvx_matrix_matrix_mpy_ahf(x) = %.7f | OK = %.7f "
                       "| matrix 1 size = %u x %u | matrix 2 size = %u x %lu | i = %lu\n",
                       (float)output[j], ref_output[j], M, K, K, i, j);
                ++fail;
            }
            else
            {
