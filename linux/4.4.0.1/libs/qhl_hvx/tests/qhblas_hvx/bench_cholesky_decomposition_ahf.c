/**=============================================================================
@file
    bench_cholesky_decomposition_ahf.c

@brief
    Benchmark for measuring performance of qhblas_hvx_cholesky_decomposition_ahf function.

    Benchmark measures number of cycles needed for executing qhblas_hvx_cholesky_decomposition_ahf function
...
int main(void)
{
    ...
    INIT_INPUT_FLOAT16(input_arr1, M * M, ranges0, RANGE_CNT0, RAND_MAX)

    performance_high();
    printf("Benchmark for qhblas_hvx_cholesky_decomposition_ahf function:\n");
    ...
    for (uint32_t i = 0; i < RANGE_VECTOR; ++i)
    {
        ...
        for (uint32_t j = 0; j < NR_OF_ITERATIONS; ++j)
        {
            ...
            measurement_start();
            qhblas_hvx_cholesky_decomposition_ahf(input_arr1, output, length);
            measurement_finish();
