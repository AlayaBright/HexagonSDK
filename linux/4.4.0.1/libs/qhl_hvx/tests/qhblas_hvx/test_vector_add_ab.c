/**=============================================================================
@file
    test_vector_add_ab.c

@brief
    Accuracy test for qhblas_hvx_vector_add_ab function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        int16_t sum = (int16_t)input_arr1[i] + (int16_t)input_arr2[i];
        sum = ((sum > 127) ? 127 : ((sum < -127) ? -128 : sum));
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A2(qhblas_hvx_vector_add_ab)

    TEST_INPUT_AH2(qhblas_hvx_vector_add_ab)
