/**=============================================================================
@file
    test_ah_vector_add_ab.c

@brief
    Accuracy test for qhblas_hvx_ah_vector_add_ab function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (int16_t)input_arr1[i] + (int16_t)input_arr2[i];
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A2(qhblas_hvx_ah_vector_add_ab)

    TEST_INPUT_AH2(qhblas_hvx_ah_vector_add_ab)
