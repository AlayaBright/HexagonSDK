/**=============================================================================
@file
    test_matrix_vector_mpy_ab.c

@brief
    Accuracy test for qhblas_hvx_matrix_vector_mpy_ab function
...
inline static void generate_ref_output(uint32_t m)
{
    for (size_t i = 0; i < N; i++)
    {
        ...
        ref_output[i] = (int8_t)((result >> 7) & 0xFF);
    }
}
...
int main(void)
{
    ...
    printf("Test for qhblas_hvx_matrix_vector_mpy_ab function:\n");

    for (uint32_t i = 1; i <= M; ++i)
    {
        generate_ref_output(i);

        qhblas_hvx_matrix_vector_mpy_ab(input_arr1, input_arr2, output, i, N);

        for (uint32_t j = 0; j < N; ++j)
        {
            if (ref_output[j] != output[j])
            {
                printf("Error:\tx = {%.7d, %.7d} | qhblas_hvx_matrix_vector_mpy_ab(x) = %.7d | OK = %.7d "
