/**=============================================================================
@file
    bench_matrix_matrix_mpy_af.c

@brief
    Benchmark for measuring performance of qhblas_hvx_matrix_matrix_mpy_af function.

    Benchmark measures number of cycles needed for executing qhblas_hvx_matrix_matrix_mpy_af function
...
{
#if __hexagon__
    hexagon_buffer_cleaninv((uint8_t *)input_mtx1, M * K * sizeof(float));
    hexagon_buffer_cleaninv((uint8_t *)input_mtx2, K * length * sizeof(float));
#endif
}
...
int main(void)
{
    ...
    printf("Benchmark for qhblas_hvx_matrix_matrix_mpy_af function:\n");
    ...
    for (uint32_t i = 0; i < RANGE_VECTOR; ++i)
    {
        ...
        for (uint32_t j = 0; j < NR_OF_ITERATIONS; ++j)
        {
            clear_cache(length);
            measurement_start();
            qhblas_hvx_matrix_matrix_mpy_af(input_mtx1, input_mtx2, output, M, K, length);
            measurement_finish();
