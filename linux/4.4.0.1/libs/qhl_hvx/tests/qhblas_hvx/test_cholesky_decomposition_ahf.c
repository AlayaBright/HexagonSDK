/**=============================================================================
@file
    test_cholesky_decomposition_ahf.c

@brief
    Accuracy test for qhblas_hvx_cholesky_decomposition_ahf function
...
static __fp16 input_mtx1[M * M];
static __fp16 output[M * M];
static __fp16 ref_output[M * M];
...
inline static void generate_input(uint32_t m)
{
    ...
    for (size_t i = 0; i < m; i++)
    {
        for (size_t j = 0; j < m; j++)
        {
            if (j <= i)
            {
                random_value = (__fp16)(lo + (hi - lo) * ((float)rand() / (float)RAND_MAX));
                if (j == i)
                    random_value *= (i + m);
...
float dotprod(__fp16 *a, __fp16 *b, int begin, int end)
{
    float sum = 0;
...
int main(void)
{
    ...
    printf("Test for qhblas_hvx_cholesky_decomposition_ahf function:\n");
    ...
    for (uint32_t i = 1; i <= M; ++i)
    {
        ...
        qhblas_hvx_cholesky_decomposition_ahf(input_mtx1, output, i);
        ...
        for (uint32_t j = 0; j < i * i; ++j)
        {
            ...
            if (error > TOLERANCE)
            {
                printf("Error: qhblas_hvx_cholesky_decomposition_ahf(x) = %.7f | OK = %.7f "
        ...
        memset(output, 0, i * i * sizeof(__fp16));
        memset(ref_output, 0, i * i * sizeof(__fp16));
    }
