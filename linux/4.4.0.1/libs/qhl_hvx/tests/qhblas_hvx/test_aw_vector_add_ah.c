/**=============================================================================
@file
    test_aw_vector_add_ah.c

@brief
    Accuracy test for qhblas_hvx_aw_vector_add_ah function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (int32_t)input_arr1[i] + (int32_t)input_arr2[i];
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A2(qhblas_hvx_aw_vector_add_ah)

    TEST_INPUT_AH2(qhblas_hvx_aw_vector_add_ah)
