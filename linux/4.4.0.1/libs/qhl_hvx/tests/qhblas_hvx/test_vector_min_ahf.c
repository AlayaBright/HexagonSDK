/**=============================================================================
@file
    test_vector_min_ahf.c

@brief
    Accuracy test for qhblas_hvx_vector_min_ahf function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = ((input_arr1[i] < input_arr2[i]) ? (float)input_arr1[i] : (float)input_arr2[i]);
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A2(qhblas_hvx_vector_min_ahf)

    TEST_INPUT_QFLOAT16_A2_T(qhblas_hvx_vector_min_ahf, 0.0f)
