/**=============================================================================
@file
    bench_ah_matrix_matrix_mpy_ab.c

@brief
    Benchmark for measuring performance of qhblas_hvx_ah_matrix_matrix_mpy_ab function.

    Benchmark measures number of cycles needed for executing qhblas_hvx_ah_matrix_matrix_mpy_ab function
...
int main(void)
{
    ...
    printf("Benchmark for qhblas_hvx_ah_matrix_matrix_mpy_ab function:\n");
    ...
    for (uint32_t i = 0; i < RANGE_VECTOR; ++i)
    {
        ...
        for (uint32_t j = 0; j < NR_OF_ITERATIONS; ++j)
        {
            clear_cache(length);
            measurement_start();
            qhblas_hvx_ah_matrix_matrix_mpy_ab(input_mtx1, input_mtx2, output, M, K, length);
            measurement_finish();
