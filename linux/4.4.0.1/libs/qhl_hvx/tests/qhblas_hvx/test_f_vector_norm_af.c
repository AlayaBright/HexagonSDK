/**=============================================================================
@file
    test_f_vector_norm_af.c

@brief
    Accuracy test for qhblas_hvx_f_vector_norm_af function
...
inline static float generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output += input_arr[i] * input_arr[i];
    }
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A1(qhblas_hvx_f_vector_norm_af)

    TEST_INPUT_SCALAR_QFLOAT32(qhblas_hvx_f_vector_norm_af, ARR_LEN, output,
                               0.001f, input_arr)
