/**=============================================================================
@file
    bench_cholesky_decomposition_ah.c

@brief
    Benchmark for measuring performance of qhblas_hvx_cholesky_decomposition_ah function.

    Benchmark measures number of cycles needed for executing qhblas_hvx_cholesky_decomposition_ah function
...
int main(void)
{
    ...
    performance_high();
    printf("Benchmark for qhblas_hvx_cholesky_decomposition_ah function:\n");
    ...
    for (uint32_t i = 0; i < RANGE_VECTOR; ++i)
    {
        ...
        for (uint32_t j = 0; j < NR_OF_ITERATIONS; ++j)
        {
            ...
            measurement_start();
            qhblas_hvx_cholesky_decomposition_ah(input_arr1, output, length);
            measurement_finish();
