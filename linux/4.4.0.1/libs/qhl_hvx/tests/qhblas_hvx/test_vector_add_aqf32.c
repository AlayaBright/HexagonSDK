/**=============================================================================
@file
    test_vector_add_aqf32.c

@brief
    Accuracy test for qhblas_hvx_vector_add_aqf32 function
...
int main(void)
{
    ...
    printf("Test for qhblas_hvx_vector_add_aqf32 function:\n");
    ...
    for (uint32_t i = 1; i <= ARR_LEN; ++i)
    {
        ...
        qhblas_hvx_vector_add_aqf32(input_arr1_qf32, input_arr2_qf32, output_qf32, i);
        ...
        for (uint32_t j = 0; j < i; ++j)
        {
            ...
            if (error > 0.1f)
            {
                printf("Error: input(x) = {%.7f, %.7f} | %s(x) = %.7f | OK = %.7f "
                       ...
                       input_arr1[j], input_arr2[j], qhblas_hvx_vector_add_aqf32, output[j],
                       ref_output[j], i, j, 0.1f);
                ++fail;
            }
            else
            {
