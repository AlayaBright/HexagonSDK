...
@brief
    Benchmark for measuring performance of qhmath_tanh_af function.

    Benchmark measures number of cycles needed for executing qhmath_tanh_af function
...
BENCH_TFUN_AF3(qhmath_tanh_af, ARR_LEN);

int main(int argc, char const *argv[])
{
    performance_high();

    printf("Benchmark for qhmath_tanh_af\n\n");

    // initialize test array
    BENCH_AF3_INPUT_INIT(ARR_LEN, RANGE_CNT0, RAND_MAX);

    TCALL(qhmath_tanh_af);
