...
#include "qhdsp_iir.h"
#include "qhdsp.h"
#include "test_data/coeffs/iir_coeffs_f.h"
#include "test_data/coeffs/iir_coeffs_h.h"
...
float output_buff_f_ASM[BLOCK_SIZE];
float ref_buff_f[BLOCK_SIZE];
float *filter_coeffs_ptr_f;

int16_t input_buff_h_C[BLOCK_SIZE];
int16_t input_buff_h_ASM[BLOCK_SIZE];
int16_t output_buff_h_C[BLOCK_SIZE];
int16_t output_buff_h_ASM[BLOCK_SIZE];
int16_t ref_buff_h[BLOCK_SIZE];
int16_t *filter_coeffs_ptr_h;

float biquad_filter_states_f_C[NUM_BIQUADS][2];
float_a8_t biquad_filter_states_f_ASM[NUM_BIQUADS][2];

#ifndef DOUBLE_PRECISION_IIR
int32_t biquad_filter_states_h_C[NUM_BIQUADS][2];
#else
int64_t biquad_filter_states_h_C[NUM_BIQUADS][2];
#endif
int64_t biquad_filter_states_h_ASM[NUM_BIQUADS][2];

void print_usage()
{
    printf("-----------------------------------------------------------------------------\n");
    printf("IIR test bench\n");
    printf("-----------------------------------------------------------------------------\n");
    printf("Author: QHML team.\n");
    printf("August 21st 2019.\n");
    printf("-----------------------------------------------------------------------------\n");
...
void parse_command_line(int argc, char **argv)
{
    ...
    for(i = 0; i < argc; i++)
    {
        ...
        if( strncmp( argv[i], "-f", 2 ) == 0 )
        {
            ...
            {
                filter_coeffs_ptr_f = *iir_coeffs_10kHz_HP_f;
                filter_coeffs_ptr_h = *iir_coeffs_10kHz_HP_h;
            }
            else if( strcmp( filter_type, "LP") == 0 )
            {
                filter_coeffs_ptr_f = *iir_coeffs_2kHz_LP_f;
                filter_coeffs_ptr_h = *iir_coeffs_2kHz_LP_h;
            }
            else    // default
            {
                filter_coeffs_ptr_f = *iir_coeffs_2kHz_LP_f;
...
int main(int argc, char *argv[])
{
    ...
    filter_coeffs_ptr_f = *iir_coeffs_2kHz_LP_f;       // set defaults
    filter_coeffs_ptr_h = *iir_coeffs_2kHz_LP_h;
    ...
    if(input_file_wav_header.audio_format == AUDIO_FMT_IEEE_Float_pt)
    {
        ...
        memset(biquad_filter_states_f_C, 0, (NUM_BIQUADS*2)*sizeof(float));
        memset(biquad_filter_states_f_ASM, 0, (NUM_BIQUADS*2)*sizeof(float));
        ...
        for(uint32_t i = 0; i < num_input_samples/BLOCK_SIZE; i++)
        {
            ...
            measurement_start();
            iir_f(input_buff_f_C, filter_coeffs_ptr_f, *biquad_filter_states_f_C, NUM_BIQUADS, BLOCK_SIZE, output_buff_f_C);
            measurement_finish();
            ...
            measurement_start();
            qhdsp_iir_af(input_buff_f_ASM, filter_coeffs_ptr_f, *biquad_filter_states_f_ASM, NUM_BIQUADS, BLOCK_SIZE, output_buff_f_ASM);
            measurement_finish();
    ...
    else if(input_file_wav_header.audio_format == AUDIO_FMT_Uncompressed_PCM)
    {
        ...
        if(input_file_wav_header.bits_per_sample == 16)
        {
            ...
            // initialize filter states
            #ifndef DOUBLE_PRECISION_IIR
            memset(biquad_filter_states_h_C, 0, (NUM_BIQUADS*2)*sizeof(int32_t));
            #else
            memset(biquad_filter_states_h_C, 0, (NUM_BIQUADS*2)*sizeof(int64_t));
            #endif
            memset(biquad_filter_states_h_ASM, 0, (NUM_BIQUADS*2)*sizeof(int64_t));

            // loop:
            for (uint32_t i = 0; i < num_input_samples/BLOCK_SIZE; i++)
            {
                // read input samples into FIR buffer
                fread(&input_buff_h_C, input_file_wav_header.bits_per_sample / 8, BLOCK_SIZE, fp_in);
                memcpy((void *)&input_buff_h_ASM, (const void *)&input_buff_h_C, BLOCK_SIZE*sizeof(int16_t));
                ...
                measurement_start();
                iir_h(input_buff_h_C, filter_coeffs_ptr_h, *biquad_filter_states_h_C, NUM_BIQUADS, BLOCK_SIZE, output_buff_h_C);
                measurement_finish();
                ...
                measurement_start();
                qhdsp_iir_ah(input_buff_h_ASM, filter_coeffs_ptr_h, *biquad_filter_states_h_ASM, NUM_BIQUADS, BLOCK_SIZE, output_buff_h_ASM);
                measurement_finish();
            ...
            cpc_C /= (double)(num_input_samples/BLOCK_SIZE);
            cpc_ASM /= (double)(num_input_samples/BLOCK_SIZE);
            printf("IIR fixed point test/bench:\n");
            printf("C implementation:\n");
            printf("    [num cycles]:\t%.2lf CPC\n", cpc_C);
            printf("    [SNR]:       \t%.2lf dB\n", min_snr_C);
