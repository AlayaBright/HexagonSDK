# simple python script for running all tests for FIR filter
...
hexagon_sim_cmd_line = "hexagon-sim -mv66 "
types_of_inputs = "16bPCM", "float"
output_file_suffixes = "_FIR_LPF_2kHz_", "_FIR_HPF_10kHz_"
...
for input_type in types_of_inputs:
    for j in range(len(types_of_filters)):
        ...
        test_cmd_line = hexagon_sim_cmd_line + "test_bench_fir_qhl -- " + \
                            "-i" + input_file + \
                            " -r" + ref_file + \
                            " -dc" + dut_file_C + \
                            " -da" + dut_file_ASM + \
