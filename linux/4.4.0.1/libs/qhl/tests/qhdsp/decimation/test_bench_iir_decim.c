...
#include "qhdsp.h"
#include "test_data/coeffs/iir_coeffs_deci_f.h"
#include "test_data/coeffs/iir_coeffs_deci_h.h"
...
int16_t output_buff_h[BLOCK_SIZE];
...
{
    printf("-----------------------------------------------------------------------------\n");
    printf("IIR DECIMATION test bench\n");
    printf("-----------------------------------------------------------------------------\n");
    printf("Author: QHML team.\n");
    printf("September 4th 2019.\n");
    printf("-----------------------------------------------------------------------------\n");
    ...
    for(i = 0; i < argc; i++)
    {
        ...
        if( strncmp( argv[i], "-f", 2 ) == 0 )
        {
            ...
            if( strcmp( decimation_factor, "2") == 0 )
            {
                ...
                filter_coeffs_ptr_f = *iir_coeffs_12kHz_LP_f;
                filter_coeffs_ptr_h = *iir_coeffs_12kHz_LP_h;
            }
            else if( strcmp( decimation_factor, "4") == 0 )
            {
                chosen_decimation_factor = 4;
                filter_coeffs_ptr_f = *iir_coeffs_6kHz_LP_f;
                filter_coeffs_ptr_h = *iir_coeffs_6kHz_LP_h;
            }
            else    // default
            {
                chosen_decimation_factor = 2;
...
int main(int argc, char *argv[])
{
    ...
    filter_coeffs_ptr_f = *iir_coeffs_12kHz_LP_f;       // set defaults
    filter_coeffs_ptr_h = *iir_coeffs_12kHz_LP_h;
    ...
    if(input_file_wav_header.audio_format == AUDIO_FMT_IEEE_Float_pt)
    {
        ...
        for(uint32_t i = 0; i < num_input_samples/BLOCK_SIZE; i++)
        {
            ...
            measurement_start();
            qhdsp_iirdecim_af(input_buff_f, filter_coeffs_ptr_f, *biquad_filter_states_f, \
                             NUM_BIQUADS, BLOCK_SIZE, chosen_decimation_factor, output_buff_f);
            measurement_finish();
        ...
        printf("IIR DECIMATION float test/bench:\n");
    ...
    else if(input_file_wav_header.audio_format == AUDIO_FMT_Uncompressed_PCM)
    {
        ...
        if(input_file_wav_header.bits_per_sample == 16)
        {
            ...
            for (uint32_t i = 0; i < num_input_samples/BLOCK_SIZE; i++)
            {
                ...
                measurement_start();
                qhdsp_iirdecim_ah(input_buff_h, filter_coeffs_ptr_h, *biquad_filter_states_h, \
                                 NUM_BIQUADS, BLOCK_SIZE, chosen_decimation_factor, output_buff_h);
                measurement_finish();
            ...
            cpc /= (double)(num_input_samples/BLOCK_SIZE);
            printf("IIR DECIMATION fixed point test/bench:\n");
