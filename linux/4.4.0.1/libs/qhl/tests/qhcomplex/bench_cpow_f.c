...
@brief
    Benchmark of qhcomplex_cpow_f function.
...
static float complex b[ARR_LEN];
...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < ARR_LEN; ++i)
    {
        ...
        real = lo + (hi - lo) * ((float)rand() / (float)RAND_MAX);
        imag = lo + (hi - lo) * ((float)rand() / (float)RAND_MAX);
        b[i] = _F_COMPLEX_(real, imag);
    }

    #ifdef BENCH_QHL
        TCALL(qhcomplex_cpow_f);
    #elif BENCH_OPENLIBM || BENCH_MUSL || BENCH_GLIBC || BENCH_HSDK
        TCALL(cpowf);
    #endif
