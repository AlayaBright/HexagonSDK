/**=============================================================================
@file
    test_matrix_inverse_af.c

@brief
    Accuracy test for qhblas_matrix_inverse_af function
...
int main(void)
{
  ...
  for(w = 0; w < NUMBER_OF_MATRIX_ITERATIONS; w++)
  {
    ...
    //
    check_flag = qhblas_matrix_inverse_af(input_data, output_data, MATRIX_N);
    if(check_flag == 0)
    {
  ...
  if (number_singular == 0)
   {
     ...
     if (qhblas_matrix_inverse_af(input_data, output_data, MATRIX_N) == -1)
     {
       number_singular = number_singular + 1;
     }
  }
