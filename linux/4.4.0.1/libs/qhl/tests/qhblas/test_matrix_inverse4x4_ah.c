/**=============================================================================
@file
    test_matrix_inverse4x4_ah.c

@brief
    Accuracy test for __qhblas_matrix_inverse4x4_ah function
...
int main(void)
{
  ...
  for(w = 0; w < NUMBER_OF_MATRIX_ITERATIONS; w++)
  {
    ...
    //
    check_flag = __qhblas_matrix_inverse4x4_ah(input_data, output_data);
  ...
  if (number_singular == 0)
   {
     ...
     if (__qhblas_matrix_inverse4x4_ah(input_data, output_data) == -1)
     {
       number_singular = number_singular + 1;
     }
  }
