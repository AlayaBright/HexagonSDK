/**=============================================================================
@file
    test_vector_hadamard_ah.c

@brief
    Accuracy test for qhblas_vector_hadamard_ah function
...
inline static void generate_ref_output(uint32_t size)
{
    ...
    {
        ref_output[i] = (((input_arr1[i] * input_arr2[i]) << 1) + 0x8000) >> 16;
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_A2(qhblas_vector_hadamard_ah)

    TEST_INPUT_AH2(qhblas_vector_hadamard_ah)
