/**=============================================================================
@file
    test_vector_scaling_ah.c

@brief
    Accuracy test for qhblas_vector_scaling_ah function
...
static int16_a8_t ref_output[ARR_LEN];
static const int16_t scale_factor = (int16_t)(0.3*32768);

static test_invalid_hscale test_data[] = {
...
#define NUM_TEST_TYPE (sizeof(test_data)/sizeof(test_invalid_hscale))

inline static void generate_ref_output(uint32_t size)
{
    for (uint32_t i = 0; i < size; ++i)
    {
        ref_output[i] = (((input_arr[i] * scale_factor) << 1) + 0x8000) >> 16;
    }
}
...
int main(void)
{
    ...
    TEST_INVALID_INPUT_SCALE(qhblas_vector_scaling_ah)

    TEST_INPUT_SCALE_INT16(qhblas_vector_scaling_ah)
