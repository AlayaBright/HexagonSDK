...
/**@ingroup func_qurt_tlb_entry_set2
  Sets the TLB entry by storing an entry at the specified location 
  in the Hexagon processor's TLB. Additional option can be passed 
  to lock the TLB entry in Hexagon processor's TLB.

  @param[in]   entry_id     TLB entry identifier.
  @param[in]   entry        64-bit TLB entry to store.
  @param[in]   lock         non-zero value indicates TLB entry needs to be locked in HW TLB
