...
#define QURT_ETM_TRACE_FILTER_HNUM6         (1<<6)  /**< Filter HNUM6. */  
#define QURT_ETM_TRACE_FILTER_HNUM7         (1<<7)  /**< Filter HNUM7. */  
...
typedef struct qurt_stm_trace_info {
   /** @cond */
   unsigned int stm_port_addr[6]; /* STM port address where trace data must be written to.*/
...
/**@ingroup func_qurt_trace_changed  
  ...
  @cond rest_dist For more information on the mask value, refer to the description of the trace_mask element in 
...
#define QURT_TRACE(str, ...) __VA_ARGS__
  /**< Function tracing is implemented with a debug macro (QURT_TRACE), which
       ...
       the text of the call itself. Compile the client source file with -D __FILENAME__
...
/**@ingroup func_qurt_etm_set_atb
  Sets the ATB bus state, to notify QuRT that the ATB bus is actively enabled or disabled.
