...
/**@ingroup driver_support_functions
  Adds a physical address range to the HLOS physpool of caller userPD.\n
   
  @param[in] client_handle  Obtained from the current invocation function (Section 3.4.1).
  @param[in] phys_addr      Starting adr of the physical address range 
  @param[in] size           size
...
/**@ingroup driver_support_functions
  ...
  @param[in] client_handle  Obtained from the current invocation function (Section 3.4.1).
  @param[in] phys_addr      Starting 36 bit adr of the physical address range 
  @param[in] size           size
...
/**@ingroup driver_support_functions
   Prepares for the direct manipulation of a possibly-untrusted buffer provided by a QDI
   client.
   This API is used to permit a trusted driver to safely access memory which has been
   provided by a possibly-untrusted client. A driver calls this function to obtain a safe buffer
   pointer for accessing the memory.
   This function performs the following security checks: \n
   ...
   In addition to the security checks the API will also lock the client mapping such that client
   will not be able to remove the mapping while the physical memory is being used by the trusted
   driver. \n

   @note1      Drivers are responsible to call qurt_qdi_buffer_unlock at appropriate time. Not 
               pairing qurt_qdi_buffer_unlock with this API would lead to resource leakages and 
   ...
   @note2      This API is supported in limited capacity when called from island mode. Safe buffer
               unmapping or user buffer unlock is not supported inside island mode.
   ...
   @param perms         Bitmask value which specifies the read or write access to be performed on the
                        ...
                        -- QDI_PERM_W -- Write access \n
                        -- QDI_PERM_RW -- Read/write access \n
   @param obuf         This is an optional parameter. Returns a pointer to the buffer address that 
                       the driver must use to access the buffer. If NULL is passed, then the API will
                       only perform security checks and not create a mapping to access user buffer in
                       a safe way.
   ...
   QURT_EOK        -- Lock operation was successful. In this case the QDI driver must use the 
...
/**@ingroup driver_support_functions
   This API is paired with qurt_qdi_buffer_lock2. A temporary overlapping mapping that was
   created for the driver will be removed. Client mapping for the user buffer will
   be unlocked. \n

   @note1      Drivers are responsible to pair this with qurt_qdi_buffer_lock. Not 
               pairing qurt_qdi_buffer_lock with this API would lead to resource leakages and 
               process exit failures. Drivers can keep track of which buffers are locked for
               a particular client, and if the client exits abruptly all the buffers can be
               unlocked on driver release invocation for the exiting client.
   ...
   @param buf           Pointer to the base address of the client buffer address.
   @param len           Buffer length (in bytes).
   @param obuf          The safe buffer address that was returned in obuf field after calling
                        qurt_qdi_buffer_lock2 api.
...
/**@ingroup driver_support_functions
    Invokes a synchornous callback for a given process with passing driver data to user PD.
  This is similar to qurt_qdi_cb_invoke_sync and allows the driver to pass arbitrary data to
  ...
  @param  prio            Priority at which callback should be executed.
                          This paraemter is optional. If -1 is passed, callback frameowrk will
                          execute callback at the priority of the API caller.
  @param  data            Driver arbitrary data to pass to user process. Memory pointed by data
                          must be accessible to user PD. Root driver can allocate such memory by
                          using qurt_mem_mmap() for example.
  ...
  Negative error code -- Callback could not be communicated to the framework.
 */
static __inline qurt_cb_result_t qurt_qdi_cb_invoke_sync_with_data( int client_handle,
                                                                    qurt_cb_data_t* cb_data,
                                                                    int prio,
                                                                    void *data,
                                                                    unsigned data_len
