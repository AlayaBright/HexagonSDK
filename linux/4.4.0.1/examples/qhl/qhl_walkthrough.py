...
unsupported_target_info = ["sm7150", "sm6125", "sm6150", "nicobar", "qcs405","qcs403"]

#******************************************************************************
# Parser for cmd line options
#******************************************************************************
parser = argparse.ArgumentParser(prog='qhl_walkthrough.py', description=__doc__, formatter_class=argparse.RawTextHelpFormatter)
...
def run_example():
	...
	qhl_SO = '{}/examples/qhl/qhl_example.so'.format(HEXAGON_SDK_ROOT)

	if not options.no_rebuild:
		print("---- Build qhl example for Hexagon ----")
		dev_conf.print_and_run_cmd('make -C ' + HEXAGON_SDK_ROOT + '/examples/qhl V=v66')
	else:
		print("---- Skip rebuilding qhl example for Hexagon ----")
	...
	dev_conf.print_and_run_cmd('adb -s '+device_number+'  wait-for-device push ' +qhl_SO+' ' + DSP_DST)
	...
	dev_conf.print_and_run_cmd('adb -s '+device_number+'  wait-for-device shell export LD_LIBRARY_PATH='+LIB_DST+':$LD_LIBRARY_PATH '+DSP_LIB_SEARCH_PATH+'='+DSP_LIB_PATH+' '+APPS_DST+'/run_main_on_hexagon '+options.domain + ' ' +DSP_DST+'/qhl_example.so 1 foo 2.0 bar')
