...
example_name = "fcvqueuetest"
unsupported_target_info = ["mannar","kamorta","sm6125","qcs403","qcs405","agatti","strait","monaco","divar"]

#******************************************************************************
# Parser for cmd line options
#******************************************************************************
parser = argparse.ArgumentParser(prog='asyncdspq_fcvqueuetest_walkthrough.py', description=__doc__, formatter_class=argparse.RawTextHelpFormatter)
...
def run_example():
	...
	libexample_name_skel=HEXAGON_SDK_ROOT +EXAMPLE_PATH+'/'+dsp_dir+'/ship/lib'+example_name+'_skel.so'
	libexample_dsp_skel=HEXAGON_SDK_ROOT +EXAMPLE_PATH+'/'+dsp_dir+'/ship/libfcvqueue_dsp_skel.so'
	...
	if not options.no_rebuild:
		print("---- Build asyncdspq_fcvqueuetest example for both Android and Hexagon ----")
		...
		print("---- Skip rebuilding asyncdspq_fcvqueuetest example for both Android and Hexagon ----")
	...
	dev_conf.print_and_run_cmd('adb -s '+device_number+'  wait-for-device push '+libexample_dsp_skel+' '+DSP_DST)
	...
	print("---- Run asyncdspq_fcvqueuetest Example ----")
