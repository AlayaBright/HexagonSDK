...
AEEResult fcvqueue_dsp_init(uint32 req_queue_handle, uint32 resp_queue_handle, uint32 *queue_handle) {
    ...
    *queue_handle = 0;
    ...
    FAILFALSE(((q->obj_fcvlib = dlopen("libfastcvadsp.so", RTLD_NOW)) != NULL),AEE_EUNABLETOLOAD);
    FAILFALSE(((q->dilate_3x3 = dlsym(q->obj_fcvlib, "fcvFilterDilate3x3u8")) != NULL),AEE_EUNABLETOLOAD);
    FAILFALSE(((q->gaussian_3x3 = dlsym(q->obj_fcvlib, "fcvFilterGaussian3x3u8")) != NULL),AEE_EUNABLETOLOAD);
    FAILFALSE(((q->median_3x3 = dlsym(q->obj_fcvlib, "fcvFilterMedian3x3u8")) != NULL),AEE_EUNABLETOLOAD);
...
static AEEResult do_process(fcvqueue_internal_t *q, fcvqueue_op_t op, int input_fd, int output_fd) {
    ...
    switch ( op ) {
        ...
        case FCVQUEUE_OP_DILATE3X3: {
            ...
            q->dilate_3x3(in_buf->buffer, in_buf->width, in_buf->height, out_buf->buffer);
            break;
        }
            
        case FCVQUEUE_OP_GAUSSIAN3X3: {
            FARF(RUNTIME_HIGH, "Op: gaussian3x3 0x%08x to 0x%08x, len %u\n", in_buf->buffer, out_buf->buffer, out_buf->size);
            q->gaussian_3x3(in_buf->buffer, in_buf->width, in_buf->height, out_buf->buffer, 1);
            break;
        }
            
        case FCVQUEUE_OP_MEDIAN3X3: {
            FARF(RUNTIME_HIGH, "Op: median3x3 0x%08x to 0x%08x, len %u\n", in_buf->buffer, out_buf->buffer, out_buf->size);
            q->median_3x3(in_buf->buffer, in_buf->width, in_buf->height, out_buf->buffer);
            break;
        }
