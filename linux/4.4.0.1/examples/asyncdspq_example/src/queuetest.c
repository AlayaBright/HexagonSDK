...
void test_asyncdspq(remote_handle64 handle) {
    ...
    if ( (err = queuetest_asyncdspq_test_reads(handle, attach_handle, 1, &lastmsg)) != 0 ) {
    ...
    printf("Queue created; attach handle 0x%08x\n", attach_handle);
    if ( (err = queuetest_asyncdspq_test_writes(handle, attach_handle, 1, 12765)) != 0 ) {
    ...
    printf("Queue created; attach handle 0x%08x\n", attach_handle);
    if ( (err = queuetest_asyncdspq_test_writes(handle, attach_handle, 10, 12765)) != 0 ) {
        error_code("queuetest_test_writes failed", err);
    }
    printf("Reading\n");
    for ( i = 0; i < 10; i++ ) {
    ...
    TEST_ERROR_CODE(queuetest_asyncdspq_adder_message_callbacks_start(handle,
                        attach_handle, resp_attach_handle));
    TEST_ERROR_CODE(sem_wait(&adder_exit_sem));
    TEST_ERROR_CODE(queuetest_asyncdspq_adder_message_callbacks_stop(handle));
    ...
    TEST_ERROR_CODE(asyncdspq_create(&queue, &attach_handle,
                                    ...
                                    NULL, 0));
    t1 = GetTime();
    TEST_ERROR_CODE(queuetest_asyncdspq_space_callback_start(handle, attach_handle, NUMADDS));
    ...
    t2 = GetTime();
    TEST_ERROR_CODE(queuetest_asyncdspq_space_callback_stop(handle));
    TEST_ERROR_CODE(asyncdspq_destroy(queue));
    t = (unsigned) (t2 - t1);
    printf("%d usec for %d loops - %u usec/operation\n", t, NUMADDS, t/NUMADDS);
}
...
static void *cancel_read_thread(void *arg) {
    ...
    DEBUGPRINT("... Got error 0x%x\n", err);
    if ( err != AEE_EEXPIRED ) {
        error_code("cancel_read_thread: Unexpected error code 0x%x", err);
    }
    return NULL;
}
...
void test_asyncdspq_cancel(remote_handle64 handle) {
    ...
    TEST_ERROR_CODE(asyncdspq_create(&q1, &a1,
                                    ASYNCDSPQ_ENDPOINT_APP_CPU, ASYNCDSPQ_ENDPOINT_CDSP,
                                    4096, error_callback, NULL, NULL, NULL, 0));
    TEST_ERROR_CODE(queuetest_asyncdspq_test_cancel_read(handle, a1));
    TEST_ERROR_CODE(asyncdspq_destroy(q1));
    ...
    TEST_ERROR_CODE(asyncdspq_create(&q1, &a1,
                                     ASYNCDSPQ_ENDPOINT_CDSP, ASYNCDSPQ_ENDPOINT_APP_CPU,
                                    4096, error_callback, NULL, NULL, NULL, 0));
    TEST_ERROR_CODE(queuetest_asyncdspq_test_cancel_write(handle, a1));
    TEST_ERROR_CODE(asyncdspq_destroy(q1));
}
...
void test_asyncdspq_threads(remote_handle64 handle) {
    ...
    TEST_ERROR_CODE(queuetest_asyncdspq_multiadder_message_callbacks_start(handle, attach_handle, resp_attach_handle));
    ...
    TEST_ERROR_CODE(queuetest_asyncdspq_multiadder_message_callbacks_stop(handle));
    ...
    TEST_ERROR_CODE(queuetest_asyncdspq_multiadder_threads_start(handle, attach_handle, resp_attach_handle,
    ...
    TEST_ERROR_CODE(queuetest_asyncdspq_multiadder_threads_stop(handle));
    t2 = GetTime();
    ...
    TEST_ERROR_CODE(queuetest_asyncdspq_multiadder_threads_start(handle, attach_handle, resp_attach_handle,
                                                                MULTIADDER_DSP_THREADS));
    printf("queuetest_asyncdspq_multiadder_threads_stop(handle)\n");
    TEST_ERROR_CODE(queuetest_asyncdspq_multiadder_threads_stop(handle));
    TEST_ERROR_CODE(asyncdspq_destroy(queue));
    TEST_ERROR_CODE(asyncdspq_destroy(resp_queue));
}
...
static int open_session( bool isUnsignedPD_Enabled, int testloops)
{
    int nErr = 0;
    ...
    while ( testloops-- ) {
        ...
        test_asyncdspq_cancel(handle);
        test_asyncdspq_threads(handle);
    }
...
{
    printf( "Usage:\n"
            "    queuetest [-d domain] [-U unsigned_PD] [-t test loops]\n\n"
            "Options:\n"
            ...
            "        Default Value: 1\n"
            "-t test_loops: Number of FastRPC calls to each DSP routine.\n"
            "        Default Value: 1\n"
            );
}
