...
AEEResult queuetest_open(const char *uri, remote_handle64 *h)
{
    queuetest_context_t *queuetest_ctx = malloc(sizeof(queuetest_context_t));

    if(queuetest_ctx==NULL)
    {
        FARF(ERROR, "Failed to allocate memory for the queuetest context");
        return AEE_ENOMEMORY;
    }

    *h = (remote_handle64) queuetest_ctx;

    return AEE_SUCCESS;
}


AEEResult queuetest_close(remote_handle64 h)
{
    queuetest_context_t *queuetest_ctx = (queuetest_context_t*) h;

    if(queuetest_ctx==NULL)
    {
        FARF(ERROR, "NULL handle received in queuetest_close()");
...
AEEResult queuetest_asyncdspq_test_reads(remote_handle64 h, uint32 attach_handle, uint32 nummsgs, uint32 *lastmsg) {
...
AEEResult queuetest_asyncdspq_test_writes(remote_handle64 h, uint32 attach_handle, uint32 nummsgs, uint32 firstmsg) {
...
AEEResult queuetest_asyncdspq_adder_spin(remote_handle64 h, uint32 req_queue_attach_handle, uint32 resp_queue_attach_handle) {
...
fail:
    ...
    FARF(RUNTIME_LOW, "queuetest_asyncdspq_adder_spin: Done (0x%x)\n", err);
    return err;
}
...
AEEResult queuetest_asyncdspq_adder_message_callbacks_start(remote_handle64 h, uint32 req_queue_attach_handle,
...
AEEResult queuetest_asyncdspq_adder_message_callbacks_stop(remote_handle64 h) {
    int err = 0;
...
AEEResult queuetest_asyncdspq_space_callback_start(remote_handle64 h, uint32 attach_handle, uint32 nummsgs) {
    int err = 0;
...
AEEResult queuetest_asyncdspq_space_callback_stop(remote_handle64 h) {
...
AEEResult queuetest_asyncdspq_multiadder_message_callbacks_start(remote_handle64 h, uint32 req_queue_attach_handle,
                                                                uint32 resp_queue_attach_handle) {
...
AEEResult queuetest_asyncdspq_multiadder_message_callbacks_stop(remote_handle64 h) {
    int err = 0;
...
AEEResult queuetest_asyncdspq_multiadder_threads_start(remote_handle64 h, uint32 req_queue_attach_handle,
                                                      uint32 resp_queue_attach_handle,
                                                      uint32 num_threads) {
...
AEEResult queuetest_asyncdspq_multiadder_threads_stop(remote_handle64 h) {
    int err = 0;
    int i;
...
AEEResult queuetest_enable_logging(remote_handle64 h) {
...
AEEResult queuetest_asyncdspq_test_cancel_read(remote_handle64 h, uint32 attach_handle) {
...
AEEResult queuetest_asyncdspq_test_cancel_write(remote_handle64 h, uint32 attach_handle) {
...
AEEResult queuetest_set_clocks(remote_handle64 h) {
    ...
    queuetest_context_t *queuetest_ctx = (queuetest_context_t*) h;
    if ( (err = HAP_power_set((void*) queuetest_ctx, &request)) != 0 ) {
        return err;
    }
    ...
    return HAP_power_set((void*) queuetest_ctx, &request);
}
