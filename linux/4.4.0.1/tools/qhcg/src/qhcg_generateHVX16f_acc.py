...
class GeneratorHVX16f_acc(object):
    ...
    def __init__(self, function, outputer, a0, b0, coeffs, poly_order):
        ...
        self.a0_fp16_str = float_to_fp16_hex(self.a0)

        self.scale = get_valid_scale_fp16(self.a0, self.b0)
        self.scale_fp16_str = float_to_fp16_hex(self.scale)
        ...
        self.key_maps = {
            ...
            "QHCG_KEY_POLY_EVAL_LOW": self.getQHCG_KEY_POLY_EVAL_LOW,
            "QHCG_KEY_POLY_EVAL_HIGH": self.getQHCG_KEY_POLY_EVAL_HIGH,
    ...
    def getQHCG_KEY_HVX_VECTOR_DECL(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'HVX_DV c{0}_coeff_dv;\n' + \
                indent_spaces + 'HVX_VectorPair c{0}_coeff_vp;\n' + \
                indent_spaces + 'HVX_Vector c{0}_coeff_v;\n'
        out_str = ''
    ...
    def getQHCG_KEY_COEFFS(self):
        for i in range(self.order + 1):
            coeff_seg = np.zeros(self.segments, dtype=float) # Make padding with 16 zeros at the beginning of array
            for j in range(self.segments):
                coeff_seg[j] = self.coeffs[j - self.segments][self.order - i]
            self.coeffs_arr.append(coeff_seg)

        one_a = 'static const float c{}_coeffs[{}] __attribute__((aligned(VLEN))) =\n' \
                '{{' \
                '{}\n' \
                '}};\n'
        out_str = ''
        for i in range(self.order + 1):
            start_zeros = np.zeros(self.segments, dtype=float)
            tmp_coeffs_arr = np.append(start_zeros, self.coeffs_arr[i])
            coeff_arr_idx = format_table_str(tmp_coeffs_arr)
...
# TODO: clear execess
    def getQHCG_KEY_SF_2_Q32(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'c{0}_coeff_v = Q6_Vqf32_vadd_VsfVsf(c{0}_coeff_v, zero_v_hf);\n'
    ...
    def getQHCG_KEY_SPLIT_COEFF(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'c{0}_coeff_dv.VV = Q6_Wuw_vzxt_Vuh(c{0}_coeff_v);\n'
        out_str = ''
        for i in range(self.order + 1):
            out_str += one_a.format(i)
    ...
    def getQHCG_KEY_VLUT(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'c{0}_coeff_vp = Q6_Wh_vlut16_VbVhR(idx1_v, Q6_V_lo_W(c{0}_coeff_dv.VV), 1);\n' + \
                indent_spaces + 'c{0}_coeff_vp = Q6_Wh_vlut16or_WhVbVhR(c{0}_coeff_vp, idx2_v, Q6_V_hi_W(c{0}_coeff_dv.VV), 1);'
        out_str = ''
        for i in range(self.order + 1):
            out_str += one_a.format(i)
            out_str += '\n'
    ...
    def getQHCG_KEY_SCALE_FACT(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'scale_v = Q6_Vh_vsplat_R({});\n'

        return one_a.format(self.scale_fp16_str)

    def getQHCG_KEY_HVX_A0_VECT(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'input_min_v_hf = Q6_Vh_vsplat_R({});\n'

        return one_a.format(self.a0_fp16_str)

    def getQHCG_KEY_POLY_EVAL_LOW(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'output_dv.V.lo = Q6_Vqf32_vmpy_Vqf32Vqf32(Q6_V_lo_W(c{idx_1}_coeff_vp), Q6_V_lo_W(input_vp_qf32));\n' + \
                indent_spaces + 'output_dv.V.lo = Q6_Vqf32_vadd_Vqf32Vqf32(output_dv.V.lo, Q6_V_lo_W(c{idx_2}_coeff_vp));\n'
        ...
        for i in reversed(range(self.order)):
            if (i == (self.order - 1)):
                ...
                one_a = indent_spaces + 'output_dv.V.lo = Q6_Vqf32_vmpy_Vqf32Vqf32(output_dv.V.lo, Q6_V_lo_W(input_vp_qf32));\n' + \
                        indent_spaces + 'output_dv.V.lo = Q6_Vqf32_vadd_Vqf32Vqf32(output_dv.V.lo, Q6_V_lo_W(c{}_coeff_vp));\n'
                out_str += one_a.format(i)

        return out_str

    def getQHCG_KEY_POLY_EVAL_HIGH(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'output_dv.V.hi = Q6_Vqf32_vmpy_Vqf32Vqf32(Q6_V_hi_W(c{idx_1}_coeff_vp), Q6_V_hi_W(input_vp_qf32));\n' + \
                indent_spaces + 'output_dv.V.hi = Q6_Vqf32_vadd_Vqf32Vqf32(output_dv.V.hi, Q6_V_hi_W(c{idx_2}_coeff_vp));\n'
        ...
        for i in reversed(range(self.order)):
            if (i == (self.order - 1)):
                ...
                one_a = indent_spaces + 'output_dv.V.hi = Q6_Vqf32_vmpy_Vqf32Vqf32(output_dv.V.hi, Q6_V_hi_W(input_vp_qf32));\n' + \
                        indent_spaces + 'output_dv.V.hi = Q6_Vqf32_vadd_Vqf32Vqf32(output_dv.V.hi, Q6_V_hi_W(c{}_coeff_vp));\n'
                out_str += one_a.format(i)
