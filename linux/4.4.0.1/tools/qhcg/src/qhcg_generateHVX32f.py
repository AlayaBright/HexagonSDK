...
from qhcg_helper import float_to_fp32_hex, get_valid_scale_fp32
...
class GeneratorHVX32f(object):

    def __init__(self, function, outputer, a0, b0, coeffs, poly_order):

        input_path = gen_data.path_generator(mode=Modes_process_output.read_input, input_types=Input_types.float32)
        output_path = gen_data.path_generator(mode=Modes_process_output.read_HVX, output_types=Output_types.float32)
        ...
        self.a0_fp32_hex = float_to_fp32_hex(self.a0)

        self.scale_fp32 = get_valid_scale_fp32(self.a0, self.b0)
        self.scale_fp32_hex = float_to_fp32_hex(self.scale_fp32)
    ...
    def getQHCG_KEY_SF_2_Q32(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'c{0}_coeff_v = Q6_Vqf32_vadd_VsfVsf(c{0}_coeff_v, zero_v_sf);\n'
    ...
    def getQHCG_KEY_SCALE_FACT(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'scale_v = Q6_V_vsplat_R({});\n'

        return one_a.format(self.scale_fp32_hex)

    def getQHCG_KEY_HVX_A0_VECT(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'input_min_v_f = Q6_V_vsplat_R({});\n'

        return one_a.format(self.a0_fp32_hex)

    def getQHCG_KEY_POLY_EVAL(self, indent_spaces: str = ''):
        one_a = indent_spaces + 'output_v = Q6_Vqf32_vmpy_Vqf32Vqf32(Q6_V_lo_W(c{idx_1}_coeff_vp), input_v_qf32);\n' + \
                indent_spaces + 'output_v = Q6_Vqf32_vadd_Vqf32Vqf32(output_v, Q6_V_lo_W(c{idx_2}_coeff_vp));\n'
        ...
        for i in reversed(range(self.order)):
            if (i == (self.order - 1)):
                ...
                one_a = indent_spaces + 'output_v = Q6_Vqf32_vmpy_Vqf32Vqf32(output_v, input_v_qf32);\n' + \
                        indent_spaces + 'output_v = Q6_Vqf32_vadd_Vqf32Vqf32(output_v, Q6_V_lo_W(c{}_coeff_vp));\n'
                out_str += one_a.format(i)
