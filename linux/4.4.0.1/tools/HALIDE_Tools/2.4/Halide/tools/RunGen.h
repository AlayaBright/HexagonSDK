...
// We open the log file here because all the subsequent rungen logs
// will be redirected to this file.
std::ofstream log_file(LOG_OUT_FILE);

extern "C" {

void halide_default_print(void *user_context, const char *str) {
    FARF(LOW, "%s\n", str);
}

halide_print_t print_handler = halide_default_print;

void halide_print(void *user_context, const char *msg) {
    (*print_handler)(user_context, msg);
}

void halide_default_error(void *user_context, const char *msg) {
    FARF(LOW, "In halide_error\n");
    halide_default_print(user_context, msg);
}

halide_error_handler_t error_handler = halide_default_error;
...
halide_print_t halide_set_custom_print(halide_print_t print) {
    halide_print_t result = print_handler;
    print_handler = print;
    return result;
}
