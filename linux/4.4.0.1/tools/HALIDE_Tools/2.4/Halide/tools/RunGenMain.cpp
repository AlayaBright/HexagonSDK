...
Known Issues:
    ...
    * Filters using buffer layouts other than planar (e.g. interleaved/chunky)
    ...
    out() << replace_all(usage, "$NAME$", basename);
}
...
#ifndef HL_HEXAGON_STANDALONE
    std::cout << s;
#else
// In hexagon standalone mode, rungen runs on the DSP which does not have
// access to write to the stdout or stderr. So we redirect all log messages
// to a file which is then read by the host side driver which calls the rungen
// DSP library and prints the contents of the log file to the console.

    if (!log_file.is_open()) {
        log_file.open(LOG_OUT_FILE, std::ios_base::app);
    }

    if (!(log_file << s.c_str())) {
        FARF(HIGH, "Error writing data:\n%s\nTo file: %s", s.c_str(), LOG_OUT_FILE);
    } else {
        FARF(LOW, "Wrote:\n%s\nTo file: %s", s.c_str(), LOG_OUT_FILE);
        log_file.close();
    }
...
void do_log_cerr(const std::string &s) {
    ...
    if (!(log_file << s.c_str())) {
        ...
        FARF(HIGH, "rungen_error: %s", s.c_str());
        log_file.close();
    }
#endif
}
...
int main(int argc, char **argv) {
    ...
    if (registered_filters == nullptr) {
        fail() << "No filters registered. Compile RunGenMain.cpp along with at least one 'registration' output from a generator.\n";
        return -1;
    }
    ...
    if (track_memory) {
        ...
        tracker.install();
#else
        fail() << "--track_memory is not supported in Hexagon Standalone mode. Aborting.\n";
#endif
    }
...
#ifdef HL_HEXAGON_STANDALONE

// Point of entry for the DSP side rungen library.
int rungen_main(const rungen_str* str, int strLen) {
    char *argv[strLen];
    for (int i = 0; i < strLen; ++i) {
        argv[i] = str[i].data;
    }
    return main(strLen, argv);
}

int rungen_power_on_hvx() {
    FARF(HIGH, "\npower_on\n");
    return power_on_hvx();
}

int rungen_power_off_hvx() {
    FARF(HIGH, "\npower_off\n");
    return power_off_hvx();
}

int rungen_set_hvx_perf_mode_low() {
    FARF(HIGH, "\nperf_low\n");
    return set_hvx_perf_mode_low();
}

int rungen_set_hvx_perf_mode_nominal() {
    FARF(HIGH, "\nperf_nominal\n");
    return set_hvx_perf_mode_nominal();
}

int rungen_set_hvx_perf_mode_turbo() {
    FARF(HIGH, "\nperf_turbo\n");
