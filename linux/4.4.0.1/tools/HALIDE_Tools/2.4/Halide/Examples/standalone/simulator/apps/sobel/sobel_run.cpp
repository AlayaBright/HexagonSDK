...
int main(int argc, char **argv) {
    ...
    cycles = benchmark([&]() {
        ...
        if (error != 0) {
            printf("sobel pipeline failed: %d\n", error);
        }
    });
    SIM_RELEASE_HVX;

    double cycles_per_pixel = (double)cycles / width / height;
    printf("AppReported: Image %dx%d - sobel(128B): %0.4f cycles/pixel\n", (int)width, (int)height, cycles_per_pixel);
    TestReport tr("sobel", cycles_per_pixel, "cyc/pix", Mode::Simulator_Standalone);
    tr.print();
    ...
    cycles_baseline = benchmark([&]() {
        int error = sobel_i(input, stride, width, height, output);
        if (error != 0) {
    ...
    SIM_RELEASE_HVX;
    printf("AppReported: Image %dx%d - sobel_baseline(128B): %0.4f cycles/pixel\n", (int)width, (int)height, (float)cycles_baseline / width / height);
