...
int main(int argc, char *argv[]) {
    ...
    int ret = dma_raw_blur_rw_async_getLinearFrameSize(width, height, stride, format, &linear_frame_size);
    if (ret != 0) {
        printf("dma_raw_blur_rw_async_getLinearFrameSize failed\n");
        error("dma_raw_blur_rw_async");
    }
    int linear_frame_height = linear_frame_size / stride;
    int ubwc_frame_size = 0;
    ret = dma_raw_blur_rw_async_getUBWCFrameSize(width, height, stride, format, &ubwc_frame_size);
    if (ret != 0) {
        printf("dma_raw_blur_rw_async_getUBWCFrameSize failed\n");
        error("dma_raw_blur_rw_async");
    }
    ...
    printf("Running pipeline\n");
    int result = dma_raw_blur_rw_async_run(input.get_buffer(), input.len(), width, height, stride, format, is_input_ubwc, is_output_ubwc, output.get_buffer(), output.len(), iterations, &avg_time);
    if (result) {
    ...
    if (!verify(input_validate, output, width, (is_output_ubwc ? ubwc_frame_height : linear_frame_height))) {
        error("dma_raw_blur_rw_async", avg_time);
    } else {
