...
#include "dma_p010_rw_async_halide.h"
...
#endif
// API to power on HVX
int dma_p010_rw_async_power_on_hvx(void) {
    FARF(HIGH, "\npower_on\n");
    return power_on_hvx();
}
// API to power off HVX
int dma_p010_rw_async_power_off_hvx(void) {
    FARF(HIGH, "\npower_off\n");
    return power_off_hvx();
}
// APIs to select HVX performance modes(Low/nomial/turbo)
int dma_p010_rw_async_set_hvx_perf_mode_low(void) {
    FARF(HIGH, "\nperf_low\n");
    return set_hvx_perf_mode_low();
}
int dma_p010_rw_async_set_hvx_perf_mode_nominal(void) {
    FARF(HIGH, "\nperf_nominal\n");
    return set_hvx_perf_mode_nominal();
}
int dma_p010_rw_async_set_hvx_perf_mode_turbo(void) {
    FARF(HIGH, "\nperf_turbo\n");
    return set_hvx_perf_mode_turbo();
}
// API to get Linear Frame Size.
int dma_p010_rw_async_getLinearFrameSize(int width, int height, int stride, int format, int *size) {
    if (!size) {
        return -1;
    }
    *size = getFrameSize(width, height, stride, eDmaFmt_P010, FALSE);
    return 0;
}

// API to get UBWC Frame Size.
int dma_p010_rw_async_getUBWCFrameSize(int width, int height, int stride, int format, int *size) {
    if (!size) {
        return -1;
    }
    *size = getFrameSize(width, height, stride, eDmaFmt_P010, TRUE);
    return 0;
}
// This function Wraps the ION buffers to halide_buffer_t and invoke the Halide Pipeline code
int dma_p010_rw_async_run(const unsigned char *input_buffer,
    ...
    // Y and UV data need to be UBWCDMA'ed separately to allow seperate processing steps
    ...
    // User format to UBWCDMA Format conversion
    ...
    // devicep points to the input buffer. input_y is a halide_buffer_t object.
    // We have already filled the relevant parameters of its dimensions (min, stride, extent).
    // halide_hexagon_dma_device_wrap_native() wraps input_y around the raw devicep
    // pointer and sets up the UBWCDMA device_interface of input_y.
    // This means setting up the frame width, height and stride for the UBWCDMA transfer.
    // We similarly wrap the output_y around the raw devicey pointer and output_uv
    // around devicec pointer.
    ...
    // Initialize UBWCDMA with input UV Frame details like width, height and stride
    ...
    uint64_t devicey;
    uint64_t devicec;
    devicey = (uint64_t)output_memory;
    devicec = (uint64_t)output_memory;
    ...
    // Initialize UBWCDMA with Output UV Frame details like width, height and stride
    // which is used for UBWCDMA config.
    output_uv.device = 0;
    ...
    // Mark the device of the input_y buffer as dirty. This will ensure that the copy
    // happens from the device (DDR) to the host (Hexagon L2 Locked Cache), that is a UBWCDMA read.
    // We do this for both input_y and input_uv.
    ...
    // We call this API for input_y and input_uv buffer.
    nRet = halide_hexagon_dma_prepare_for_copy_to_host(0, &input_y, dma_engine, is_input_ubwc_flag, linear_format);
    if (nRet) {
        // De-allocate the Allocated UBWCDMA engine if the API fails
    ...
    nRet = halide_hexagon_dma_prepare_for_copy_to_host(0, &input_uv, dma_engine, is_input_ubwc_flag, chroma_format);
    if (nRet) {
        // De-allocate the Allocated UBWCDMA engine if the API fails
        FARF(HIGH, "halide_hexagon_dma_prepare_for_copy_to_host returns %d \n", nRet);
        halide_hexagon_dma_deallocate_engine(0, dma_engine);
    ...
    // Mark the device of the output_y buffer as dirty. This will ensure that the copy
    // happens from the host (Hexagon L2 Locked Cache) to the device (DDR), that is a UBWCDMA write.
    // We do this for both output_y and output_uv.
    ...
    // DMA format, direction of the UBWCDMA transfer (write), whether output image is linear
    // or compressed and adjusts the UBWCDMA frame parameters based on Image format provided.
    // We call this API for output_y and output_uv buffer.
    nRet = halide_hexagon_dma_prepare_for_copy_to_device(0, &output_y, dma_engine, is_output_ubwc_flag, linear_format);
    if (nRet) {
        // De-allocate the Allocated UBWCDMA engine if the API fails
    ...
    nRet = halide_hexagon_dma_prepare_for_copy_to_device(0, &output_uv, dma_engine, is_output_ubwc_flag, chroma_format);
    if (nRet) {
        // De-allocate the Allocated UBWCDMA engine if the API fails
        FARF(HIGH, "halide_hexagon_dma_prepare_for_copy_to_device returns %d \n", nRet);
        halide_hexagon_dma_deallocate_engine(0, dma_engine);
    ...
    (*avg_time) = benchmark(iterations, [&]() {
        result = dma_p010_rw_async_halide(&input_y, &input_uv, &output_y, &output_uv);
        if (result != 0) {
            FARF(HIGH, "dma_p010_rw_test failed! %d\n", result);
            return result;
        }
    });
    // de-init UBWCDMA : Input Y- Frame
    ...
    // de-init UBWCDMA : Output Y- Frame
    ...
    // de-init UBWCDMA : Input  UV- Frame
    ...
    // de-init UBWCDMA : Output UV - Frame
