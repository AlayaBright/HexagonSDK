...
class MorphedgeDescriptor : public PipelineDescriptorBase {
    Halide::Runtime::Buffer<uint8_t> u8_in, u8_out;
...
public:
    MorphedgeDescriptor(int W, int H)
        : u8_in(nullptr, W, H),
          u8_out(nullptr, W, H) {
    }
    ...
    bool verify(const int W, const int H) {
        ...
        u8_out.for_each_element([&](int x, int y) {
            auto u8_in_bounded = [&](int x_, int y_) { return u8_in(clamp(x_, 0, W - 1), clamp(y_, 0, H - 1)); };
            uint8_t max_y[3], min_y[3];
            max_y[0] = max3(u8_in_bounded(x - 1, y - 1), u8_in_bounded(x - 1, y), u8_in_bounded(x - 1, y + 1));
            max_y[1] = max3(u8_in_bounded(x, y - 1), u8_in_bounded(x, y), u8_in_bounded(x, y + 1));
            max_y[2] = max3(u8_in_bounded(x + 1, y - 1), u8_in_bounded(x + 1, y), u8_in_bounded(x + 1, y + 1));
            uint8_t max_val = max3(max_y[0], max_y[1], max_y[2]);
            min_y[0] = min3(u8_in_bounded(x - 1, y - 1), u8_in_bounded(x - 1, y), u8_in_bounded(x - 1, y + 1));
            min_y[1] = min3(u8_in_bounded(x, y - 1), u8_in_bounded(x, y), u8_in_bounded(x, y + 1));
            min_y[2] = min3(u8_in_bounded(x + 1, y - 1), u8_in_bounded(x + 1, y), u8_in_bounded(x + 1, y + 1));
            uint8_t min_val = min3(min_y[0], min_y[1], min_y[2]);
            uint8_t n = (max_val - min_val) <= 25 ? 0 : (max_val - min_val);
            uint8_t m = clamp<uint8_t>(2 * n, 0, 255);
            uint8_t out_xy = u8_out(x, y);
            if (m != out_xy) {
                printf("Morph_edge: Mismatch at %d %d : %d != %d\n", x, y, out_xy, m);
                abort();
            }
        });
        return true;
    ...
    int run() {
#ifdef MORPH_EDGE
        return morph_edge(u8_in, u8_out);
#endif
        return 1;
    }
    void finalize() {
