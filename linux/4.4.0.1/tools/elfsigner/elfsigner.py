...
from sectools.features.isc.parsegen.elf_support.elf_vars import set_non_encap_segments
from sectools.features.isc.parsegen.elf_support.elf_vars import set_non_hashable_segments
...
class ElfSignerCore(object):
    ...
    def generate_config_file(self, src_config, dest_config, args):
        config = SecImageConfigParser(src_config)
 ...
 """\nExecute command:
    ...
    def _executeCmds(self, cmds, verbose):
        ...
        try:
            ...
            backup_non_hashable_segments = elf_vars[NON_HASHABLE_SEGMENTS]
            set_non_hashable_segments([])

            #Incluse phdr as non-encap segment
            backup_non_encap_segments = elf_vars[NON_ENCAP_SEGMENTS]
            set_non_encap_segments(backup_non_encap_segments + [PT_PHDR])

            # Call sectools
            image_info_list = secimage.main(parsed_args)[1]
