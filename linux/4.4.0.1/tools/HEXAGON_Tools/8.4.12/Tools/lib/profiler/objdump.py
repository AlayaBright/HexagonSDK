...
def read_disassembly(cmd, fn_elf, offset, procs):
    packet_pc = 0
    packet = []
    hll_linenum = ''
    addl_opts = get_addl_objdump_opts()

    has_hmx = 'hmx' in [proc.lower().strip() for proc in procs]
    hmx = '-mattr=+hmx' if has_hmx else ''
    cmd = '{cmd} -line-numbers -print-imm-hex {addl_opts} {fn_elf} {hmx}'.format(**locals())
...
def get_info(fn_elf, tools_dir, verbose, processors):
    import os.path
    ...
    for arg in fn_elf.split(','):
        ...
        if not read_funcs (cmd, fn, offset):
            ...
            if not read_funcs (objdump_command, fn, offset):
                ...
                read_disassembly (objdump_command, fn, offset, processors)
        else:
            if verbose:
                print ('Disassembler: %s' % cmd)
            read_disassembly (cmd, fn, offset, processors)
