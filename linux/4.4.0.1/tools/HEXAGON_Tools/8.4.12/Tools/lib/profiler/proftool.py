    ...
    $(function () {
        $('#tabs').tabs({
            beforeActivate: function(event, ui) {
                ...
                if ((curTab == 'CORE') || (curTab == 'HVX') || (curTab == 'HMX')
                    || (curTab == 'events')) {
                    SaveButtonState (curTab);
                }

                if (newTab == 'CORE') {
                    if (curTab != 'HVX' || curTab != 'HMX') {
                        $('#DX1').show();
                        $('#DX2').show();
                        $('#DX3').show();
                        $('#DX4').show();
        ...
        var tabsWithButtons = ['CORE', 'HVX', 'HMX', 'events'];
...
def gen_stallinfo(f,name,stalltotal,pc_to_func,stats,is_fa,name_to_pc,name_to_func, processor):
    ...
    f.write ('''            <table>
              <tr>
                <th colspan=4 style="padding-top:10px;">{displayname}</th>
                <th></th>
              </tr>
              <tr>
                <td class="sm_hdr">#</td>
                <td class="sm_hdr_lpad" title="Count: the raw count of the sum of stalls+commits for this addr/function">Count</td>
                <td class="sm_hdr_lpad" title="Pct: the percentage of the stalls due to this location in isolation">Pct</td>
                <td class="sm_hdr_lpad" title="CumPct: the percentage of the cumulative stalls due to this location and ones with fewer stalls">CumPct</td>
                <td class="sm_hdr_lpad" title="{addr_hdr}: program location address">{addr_hdr}</td>
                <td class="sm_hdr_lpad" style="text-align:left;" title="{which_hdr}: symbolic location">{which_hdr}</td>
              </tr>
'''.format(**locals()))
...
def do_main(cmd,flag,fn_json,fn_elf,fn_html,tools_dir,vStr,slzStr,s0xStr,toolVersion,braceStr,jtfStr,fixChromeMagStr,dockStr,ulJumpsStr,hiJumpsStr,selStr):
    ...
    objdump.get_info(fn_elf, tools_dir, verbose, processors)
    ...
    # Tracking this is a maintenance challenge, disabled:
    toolVersion = ''
