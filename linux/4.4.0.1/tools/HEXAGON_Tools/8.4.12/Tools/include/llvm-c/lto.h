...
/**
 * Returns the size of the ith symbol in the object module.
 *
 * \since prior to LTO_API_VERSION=3
 */
extern size_t
lto_module_get_symbol_size(lto_module_t mod, unsigned int index);
...
/**
 * If symbol has section associated with it, get section name.
 * Otherwise return null.
 *
 * \since prior to LTO_API_VERSION=3
 */
extern const char*
lto_module_get_symbol_section_name(lto_module_t mod, unsigned int index);


/**
 * Set final section for the symbol. Intended to communicate linker script
 * settings back to the compiler.
 *
 * \since prior to LTO_API_VERSION=3
 */
extern void
lto_module_set_symbol_section_name(lto_module_t mod, unsigned int index,
                                   const char *name);

/**
 * Return number of local uses for an external symbol.
 *
 * \since prior to LTO_API_VERSION=3
 */
extern size_t
lto_module_get_symbol_reference_count(lto_module_t mod,
                                      unsigned int symbol_index);

/**
 * Return name of the external dependencies for a symbol.
 * This API is intended to be called in a while() loop with increasing
 * dep_index count for a fixed symbol_index. It will return symbol name
 * if given dependency exist. This mean that if the following is true:
 * if(!lto_module_get_symbol_reference(mod, 1, 0))
 * symbol at index '1' has no external dependencies in mod.
 *
 * \since prior to LTO_API_VERSION=3
 */
extern const char *
lto_module_get_symbol_reference(lto_module_t mod, unsigned int symbol_index,
                                unsigned int dep_index);

extern unsigned int
lto_module_get_num_sections(lto_module_t mod);

extern const char*
lto_module_get_section_name(lto_module_t mod,
                            unsigned int section_index);

/**
 * Initialize the LTO CodeGenerator with target-specific and command line
 * options.
 *
 * This implements the 'driver' for LTOCodeGenerator when used from within the
 * linker and consolidates all non-standard options that are used for LTO at QC.
 */
extern void
qc_lto_driver_init(lto_code_gen_t cg, const char *triple, const char *options);
