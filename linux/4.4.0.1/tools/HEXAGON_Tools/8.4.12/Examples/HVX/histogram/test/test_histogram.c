...
int main(int argc, char* argv[])
{
    ...
    int           *output = memalign(VLEN, 256*sizeof(int));
    ...
    histogram( input, stride, width, height, output );

    total_cycles = READ_PCYCLES() - start_time;
    DUMP_PMU();

#if defined(__hexagon__)
    printf("AppReported (HVX%db-mode): Image %dx%d - histogram: %0.4f cycles/pixel\n", VLEN, (int)width, (int)height, (float)total_cycles/width/height);
#endif
    /* -----------------------------------------------------*/
