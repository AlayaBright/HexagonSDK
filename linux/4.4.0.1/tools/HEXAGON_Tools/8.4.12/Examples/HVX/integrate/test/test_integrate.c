...
int main(int argc, char* argv[])
{
    ...
    unsigned int  *output = memalign(VLEN, stride*height*sizeof(unsigned int ));
    ...
    IntegrateImage( input, stride, width, height, output, stride );

    total_cycles = READ_PCYCLES() - start_time;
    DUMP_PMU();

#if defined(__hexagon__)
    printf("AppReported (HVX%db-mode): Image %dx%d - 2D Integrate: %0.4f cycles/pixel\n", VLEN, (int)width, (int)height, (float)total_cycles/width/height);
#endif
    /* -----------------------------------------------------*/
