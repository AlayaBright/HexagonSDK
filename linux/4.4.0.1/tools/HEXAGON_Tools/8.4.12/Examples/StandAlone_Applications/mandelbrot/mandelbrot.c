...
/*
 * The new v67t arch only has two threads so in order to guarantee that
 * the examples works, we must set the number of compute threads to one.
 * If you are compiling for a core with more than two hardware threads
 * then this value can be set to one less than the number of hardware
 * threads.
 */
