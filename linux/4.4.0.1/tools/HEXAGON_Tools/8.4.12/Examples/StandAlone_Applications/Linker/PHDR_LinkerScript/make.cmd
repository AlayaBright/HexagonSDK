...
@IF "%1"=="script" set LINKFLAGS=-Wl,--section-start,.start=0xdf0000,-T%SRC_DIR%/plink.script,-Map,%BIN_DIR%/link.map,--gc-sections
@IF "%2"=="script" set LINKFLAGS=-Wl,--section-start,.start=0xdf0000,-T%SRC_DIR%/plink.script,-Map,%BIN_DIR%/link.map,--gc-sections
...
:Read
@echo.
hexagon-llvm-readelf -e %BIN_DIR%\%MAIN%.elf
@IF "%1"=="read" GOTO End
