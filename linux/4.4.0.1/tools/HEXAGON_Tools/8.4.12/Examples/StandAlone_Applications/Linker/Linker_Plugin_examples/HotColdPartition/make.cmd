...
@set PLUGIN_NAME=HotColdPartition
...
@set OBJS=(text_sw, compress, main)
...
@IF EXIST %TARGET_DIR%\pmu* @DEL %TARGET_DIR%\pmu*
@IF "%1"=="clean" GOTO End
...
%CC% %CFLAGS% %TARGET_DIR%\*.o -o %TARGET_DIR%\main.elf -Wl,-T,%TARGET_DIR%\linker.script,--trace=plugin

@echo.
@echo The .text_sw section was partitioned into two sections
@echo hexagon-llvm-readelf -S %TARGET_DIR%\main.elf ^| findstr "text_sw"
hexagon-llvm-readelf -S %TARGET_DIR%\main.elf | findstr "text_sw"
@echo.
@echo The .compress section was re-ordered to put the cold sections at the end
@echo hexagon-llvm-readelf -s %TARGET_DIR%\main.elf ^| findstr "comp_"
hexagon-llvm-readelf -s %TARGET_DIR%\main.elf | findstr "comp_"
@echo.
