...
class DLL_A_EXPORT ReplaceBlockPlugin : public ControlFileSizePlugin {

public:
  // This constructor will take the plungin name
  ReplaceBlockPlugin() : ControlFileSizePlugin("REPLACEBLOCK"), Blocks() {}
  ...
  Status Run(bool Verbose) override {
    ...
    // keep the same block name
    newBlock.Name = Blocks.front().Name;
...
#define GZIP_ENCODING 16
    ...
    std::cout << "\nReplacing Block " << newBlock.Name.c_str()
              << " with a size of " << newBlock.Size << " bytes.\n\n";
  ...
  std::string GetName() override { return "REPLACEBLOCK"; }
...
bool DLL_A_EXPORT RegisterAll() {
  ThisPlugin = new ReplaceBlockPlugin();
  return true;
}
